...
class ViewADGRN extends Component {
    ...
    constructor(props){
        ...
        this.state = {
            ...
            columns: [
                {
                    name: 'grn_no', // field name in the row object
                ...
                {
                    name: 'Donation Country', // field name in the row object
                    label: 'Donation Country', // column title that will be shown in table
                    options: {
                        filter: true,
                        display: true,
                        width: 10,
                        customBodyRender: (value, tableMeta, updateValue) => {
                            return this.state.data[tableMeta.rowIndex]?.Donation?.donation_country
                        }
                    }
                },
                {
                    name: 'donation reg no', // field name in the row object
                    label: 'Donation Register Number', // column title that will be shown in table
                    options: {
                        filter: true,
                        display: true,
                        width: 10,
                        customBodyRender: (value, tableMeta, updateValue) => {
                            return this.state.data[tableMeta.rowIndex]?.Donation?.donation_reg_no
                        }
                    }
                },
                // {
    ...
    async LoadData() {
        ...
        filterData.type = "Donation GRN"
        // let id = this.props.match.params.id
        let res = await DonarService.getGRNItems(filterData)
             if (res.status == 200) {
            this.setState({
...
if (200 == res.status) {
    ...
    render(){
        ...
        return(
            <MainContainer>
                <LoonsCard>
                    <CardTitle title="View Donations Items AD" />
