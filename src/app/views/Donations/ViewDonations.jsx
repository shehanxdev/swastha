    ...
    constructor(props) {
        ...
        this.state = {
            ...
            columns: [
                ...
                {
                    name: 'co_ordinated_by', // field name in the row object
                    label: 'Co-ordinated By', // column title that will be shown in table
                    options: {
                        filter: true,
                        display: true,
                        width: 10,
                        ...
                        customBodyRenderLite: (dataIndex) => {
                            ...
                            return (
                                <Grid className="flex items-center">
                                    ...
                                    <IconButton
                                        onClick={() => {
                                            window.location.href = `/donation/view-donation-items/${id}/${donar_id}?reg_no=${this.state.data[dataIndex].donation_reg_no}`
                                        }}
                                        className="px-2"
                                        size="small"
...
//         this.setState({ filterData,activeTabIndex: 1}, () => { this.setPage(0) }) //setTab for submitted
//     }
//     else if (newValue === 2) {
//         filterData.status = 'Rejected to CIU' // status for rejected
//         this.setState({ filterData,activeTabIndex: 2}, () => { this.setPage(0) }) //setTab for rejected
//     }
//     else {
//         filterData.status = 'All Donations to CIU' // status for all donations
//         this.setState({ filterData,activeTabIndex: 3 }, () => { this.setPage(0) }) //setTab for all donations
//       }
            ...
            <MainContainer>
                <LoonsCard>
                    ...
                    {/* <Tabs
                        ...
                        onChange={(event, newValue) => {this.setTab(newValue)}}
                    ...
                    <Grid item lg={12} className=" w-full mt-2">
                            ...
                            <DatePicker
                                ...
                                value={
                                   this.state.filterData.received_date
                                }
                                // views={['year']}
                                // inputFormat="yyyy"
                                // format="yyyy"
                                onChange={(date) => {
                                    let filterData = this.state.filterData
                                    filterData.received_date = dateParse(date)
                                    this.setState({
                                        filterData,
                                    })
                                }}
                            />
                                ...
                                value={
                                   this.state.filterData.donors_invoice_date
                                }
                                // views={['year']}
                                // inputFormat="yyyy"
                                // format="yyyy"
                                onChange={(date) => {
                                    let filterData = this.state.filterData
                                    filterData.donors_invoice_date = dateParse(date)
                                    this.setState({
                                        filterData,
                                    })
                                }}
