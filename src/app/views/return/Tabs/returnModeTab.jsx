...
class ReturnMode extends Component {
        ...
        this.state = {
            ...
            columns: [
                ...
                },
                {
                    ...
                    label: 'Short Expiry Batch No', // column title that will be shown in table
                    options: {
                        filter: true,
                        display: true,
                        width: 10,
                ...
                {
                    name: 'batch_qty', // field name in the row object
                    label: 'Short Expiry Batch Qty', // column title that will be shown in table
                    options: {
                        filter: true,
                        display: true,
                        width: 10,
                ...
                },
                {
                    ...
                    label: 'Short Expiry Date', // column title that will be shown in table
                    options: {
                        filter: true,
                        display: true,
                        customBodyRender: (value, tableMeta, updateValue) => {
    ...
    componentDidMount() {
        ...
        if (localStorageService.getItem('Selected_Warehouse')) {
            let warehouseId =
                localStorageService.getItem('Selected_Warehouse')?.id
            this.props.getAllMovingAndNonMovingItems({
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (
            nextProps.wareHouseStatusModal === true ||
            nextProps.wareHouseStatusModal === false
        ) {
            this.setState({
                warehouse_id: localStorageService.getItem('Selected_Warehouse')
                    ? localStorageService.getItem('Selected_Warehouse').id
                    : '',
            })
        }
    ...
    handlechange = (data, name) => {
        if (data && typeof data !== 'string') {
            ...
            if (name === 'consumabletTmePeriod') {
                ...
                this.setState({
                    consumabletTmePeriod: data,
                    consumabletTmePeriodtoDate: moment(new Date()).format(
                        'YYYY-MM-DD'
                    ),
                    consumabletTmePeriodFromDate: moment(
                        todayDate.setMonth(
                            todayDate.getMonth() - parseInt(data)
                        )
                    ).format('YYYY-MM-DD'),
                })
            } else {
        ...
        let params = {
            ...
            group_id:
                this.state.currentGroup !== ''
                    ...
                    : null,
            short_expiry:
                this.state.shortExpiry !== '' ? this.state.shortExpiry : null,
            from_date:
                this.state.consumabletTmePeriodFromDate !== ''
                    ? this.state.consumabletTmePeriodFromDate
                    : null,
            to_date:
                this.state.consumabletTmePeriodtoDate !== ''
                    ? this.state.consumabletTmePeriodtoDate
                    : null,
            page: this.state.page,
            limit: this.state.rowsPerPage,
    ...
    handlePaginations = (page, limit) => {
        ...
        this.setState({ page, rowsPerPage: limit, loading: true }, () =>
            this.props.getAllMovingAndNonMovingItems(params)
        )
    }
                        ...
                        <Grid container className="mt-5">
                            ...
                            >
                                ...
                                <Autocomplete
                                        disableClearable
                                    className="w-full"
                                    options={this.state.categories.sort((a,b)=> (a.description?.localeCompare(b.description)))}
                            ...
                            >
                                ...
                                <Autocomplete
                                        disableClearable
                                    className="w-full"
                                    options={this.state.classes.sort((a,b)=> (a.description?.localeCompare(b.description)))}
                                    getOptionLabel={(option) =>
                                        option.description
                                    }
                                    onChange={(event, data) =>
                                        this.handlechange(data, 'currentClass')
                                    }
                            ...
                            >
                                ...
                                <Autocomplete
                                        disableClearable
                                    className="w-full"
                                    options={this.state.group.sort((a,b)=> (a.name?.localeCompare(b.name)))}
                                    getOptionLabel={(option) =>
                                        option.name ? option.name : ''
                                    }
                                    onChange={(event, value) =>
                                        this.handlechange(value, 'currentGroup')
                                    }
                                    ...
                                    <tbody>
                                        <tr>
                                            <td>
                                                <TextValidator
                                                    className=" w-full"
                                                    placeholder="consumable time period"
                                                    name="shortExpiry"
                                                    InputLabelProps={{
                                                        shrink: false,
                                                    }}
                            ...
                            >
                                <SubTitle
                                    title={'Short Expiry Less than'}
                                ></SubTitle>
                                <TextValidator
                                    className=" w-full"
                                    placeholder="Short Expiry "
                                    name="shortExpiry"
                                    InputLabelProps={{
                                        shrink: false,
                                    }}
...
const mapDispatchToProps = (dispatch) => {
    return {
        getAllMovingAndNonMovingItems: (params) =>
            getAllMovingAndNonMovingItems(dispatch, params),
...
const mapStateToProps = ({ returnReducer }) => {
    return {
        moving_nonmovingItemsStatus:
            returnReducer.movingAndNonMovingItemsStatus,
...
export default withRouter(
    connect(mapStateToProps, mapDispatchToProps)(ReturnMode)
)
