...
class ReturnMode extends Component {
...

        {
          ...
          options: {
            ...
            customBodyRender: (value, tableMeta, updateValue) => {
              return (
                <>
                  <span>{value.createdAt? moment(new Date(value.createdAt)).format("YYYY-MM-DD"):"N/A"}</span>
                </>
              )
            },
          },
            ...
            customBodyRender: (value, tableMeta, updateValue) => {
              return (
                <IconButton
                  ...
                  onClick={null}
                >
                  <RemoveRedEyeIcon onClick={() => this.props.history.push(`/return/requests/${value}`)}>mode_view_outline</RemoveRedEyeIcon>
                </IconButton>
              )
            },
          },
  ...
  componentDidMount() {
    if (localStorage.getItem("Selected_Warehouse")) {
      ...
      this.props.getAllReturnRequests({ page: this.state.page, limit: this.state.limit, from: warehouseId });
      this.props.getDrugStoreDetails({},localStorageService.getItem("userInfo")?.type,localStorageService.getItem("Selected_Warehouse")?.owner_id);
  ...
  handleFilterButton = () => {
    ...
    let params = {
      ...
      to: this.state.drugStore !== "" ? this.state.drugStore.id : null,
            ...
            <Grid container className='mt-5'>
              ...
              <Grid item lg={3} md={3} sm={3} xs={3} className="px-2">
                ...
                <Autocomplete
                                        disableClearable
                  className="w-full"
                  options={this.state.drugStoreOptions.sort((a,b)=> (a.name?.localeCompare(b.name)))}

                  getOptionLabel={(option) =>
                    option.name
                  }
                  value={this.state.drugStore}
                  onChange={(event, data) => { if (data) { this.setState({ drugStore: data }) } else { this.setState({ drugStore: "" }) } }}
                  renderInput={(params) => (
                    <TextValidator
                      {...params}
              ...
              <Grid item lg={3} md={3} sm={3} xs={3} className="px-2">
                ...
                &ensp;<Button
                  ...
                  onClick={() => {
                    ...
                    }, () => this.props.getAllReturnRequests({ page: this.state.page, limit: this.state.limit, warehouse_id: this.state.warehouseId }))
                  }}
                >
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    getDrugStoreDetails: ({},type,id) => getDrugStoreDetails(dispatch,{},type,id)
  }
}
