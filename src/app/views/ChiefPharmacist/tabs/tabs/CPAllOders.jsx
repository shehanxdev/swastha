...
class CPAllOrders extends Component {
    constructor(props) {
        super(props)
        this.state = {
                            ...
                            return (
                                <>
                                    ...
                                    <IconButton
                                        ...
                                        onClick={() => {
                                            window.location = `/chiefPharmacist/individualOrder/${this.state.data[
                                                    tableMeta.rowIndex
                                                ].id
                                                }/${this.state.data[
                                                    tableMeta.rowIndex
    ...
    async getOwnIdList() {
        ...
        // get logged user district
        var info = await localStorageService.getItem('login_user_pharmacy_drugs_stores');
        console.log('district_info', info[0]?.Pharmacy_drugs_store?.district)
        ...
        if (res.status == 200) {
            ...
            let dataSet = res.data.view.data
    ...
    async loadData() {
        ...
        let owner_id = await localStorageService.getItem('owner_id')
        filterData.from_owner_id = this.state.userOwnidList
        filterData.from_not_equal = true
        filterData.type = ['Order', 'RMSD Order', 'Return']
        ...
        let cards = await ChiefPharmacistServices.getCards(
            { type: 'Order', owner_id: this.state.userOwnidList },
            0
        )
        if (cards.status == 200) {
