...
class NPDrugApprovalPending extends Component {
    constructor(props) { 
        super(props)
        this.state = {
            remarks: [],
            ...
            columns: [
                ...
                },
                {
                    ...
                    options: {
                        ...
                        customBodyRender: (value, tableMeta, updateValue) => {
                             ...
                             console.log("Hello - 1: ", NPDrugApprovalStatus) 
                            let disbaleApprove = true
                ...
                },
                {
                    ...
                    options: {
                        ...
                        customBodyRenderLite: (dataIndex) => {
                            console.log('checking status', this.state.data[dataIndex])
...

                {
                    ...
                    options: {
                        filter: false,
                        // display: this.state.userType === "MSDSCO",
                        customBodyRenderLite: (dataIndex) => {
                            ...
                            return (
                                <Grid className="w-full" style={{ width: '250px' }}>
                                    <TextValidator
                                        ...
                                        name="ApprovedQty"
                                        InputLabelProps={{ shrink: false }}
                                        value={this.state.data[dataIndex]?.approved_quantity}
                                        disabled={this.state.userType !== "MSDSCO"}
                                        type="text"
                                        variant="outlined"
                                        size="small"
                                        onChange={(e) => {
                                            let data = this.state.data
                                            this.state.data[dataIndex].approved_quantity = e.target.value
                                            this.setState({ data })
                                        }}
                                        validators={[
                                            'required',
                            ...
                            return (
                                <Grid className="w-full" style={{ width: '250px' }}>
                                    <TextValidator
                                        ...
                                        onChange={(e) => {
                                            let data = this.state.data
                                            this.state.data[dataIndex].remark = e.target.value || null
                                            this.setState({ data })
        ...
        if (approveStatus != null) {
            if (this.state.data[index].suggested_quantity === null || this.state.data[index].suggested_quantity === undefined) {
                ...
                if (this.state.userType == "MSDCP") {
                    ...
                    console.log("Hello 123", this.state.data[index])
                    data = {
                        "type": "Approved",
                        "remark": this.state.data[index]?.remark,
                        "requested_by": this.state.data[index].requested_by,
                        "status": approveStatus,
                        "owner_id": owner_id,
                        "approved_quantity": this.state.data[index]?.approved_quantity || this.state.data[index]?.suggested_quantity,
                    }
    ...
    handleBulkApproval = async () => {
        ...
        Ids.map((x, index) => {
            ...
            if (this.state.userType == "MSDSCO") {
                if (qty_data.suggested_quantity) {
                    console.log("selected data-1", activity_data)
                    activity_data.push({
                        id: x,
                        quantity: qty_data.suggested_quantity,
                        approved_quantity:qty_data.approved_quantity || qty_data.suggested_quantity
                    })
                } 
            } else {
                if (qty_data.suggested_quantity) {
                    ...
                    activity_data.push({
                        id: x,
                        quantity: qty_data.suggested_quantity,
                        approved_quantity:qty_data.approved_quantity
                    })
                } 
        ...
        // console.log("selected data-3", this.state.data[0]?.approved_quantity)
        ...
        let data = {
            ...
            // "approved_quantity": this.state.data[0]?.approved_quantity,
        }

        console.log("selected data-3", data)
...
// --------------------------------------------
    ...
    getStatus = async () => {
        ...
        console.log('checking status',this.state.userType )
    ...
    getData = async () => {
        ...
        if (user.roles[0] == "Hospital Director" || user.roles[0] == "Hospital Admin") {
            ...
            console.log("Hospital Director logged", this.state.columns)
        }
        ...
        if (this.state.userType == "Consultant") {
            params = {
                ...
                status:['Pending', 'Director Approve', 'CP Approve', 'SCO Approve', 'AD_MSD Approve', 'D_MSD Approve', 'DDG_MSD Approve', 'DDHS Approve', 'Secretary Approve']
        ...
        } else if (this.state.userType =='Hospital Director') {
            params = {
                owner_id: tempOwnerId,
        ...
        console.log('cheking params', params)
        let res = await PrescriptionService.fetchNPRrequests({ ...params, ...this.state.tablePagination })

        if (res.status === 200){
            console.log("Hey: ", res)

            //  let filterData = res.data.view.data.filter((e)=> user.roles[0] === e?.Employee?.type)
            //  let filterData = res.data.view.data.filter((e)=> e?.status === "Pending")
    ...
    componentDidMount = async () => {
        ...
        this.setState({
            ...
            console.log('ddssdahssdahsdbas', this.state.userRole)
            this.getData()
        })
                        ...
                        {this.state.loaded ?
                            ...
                            >
                                ...
                                <Grid
                                    ...
                                    spacing={2}
                                >
                                    {/* {this.state.userRole !== "MSD AD" || this.state.userRole !== "HSCO" || this.state.userRole !== "MSD Director" || this.state.userRole !== "MSD DDG" || this.state.userRole !== "DGHS" || this.state.userRole !== "Secretary" ? */}
                                    <Grid
                                        item
                                        sx={12}
                                        md={6}
                                        ...
                                        {this.state.userType !== "MSDSCO" ?
                                            ...
                                            >
                                                <Button
                                                    ...
                                                    disabled = {!this.state.approve_enable}
                                                >
                                                    Approve All
                                                </Button>
                                            </Grid>
...
export default withStyles(styleSheet)(NPDrugApprovalPending);