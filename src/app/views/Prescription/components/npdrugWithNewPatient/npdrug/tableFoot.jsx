...
const AddTextInput = ({ options, onChange = (e) => e, val = "", solo = false, text = "Add", tail = null }) => (
        ...
        <div className={classes.tbleFooter}>
            ...
            <div className={classes.max}>
                <AddInput options={drugs} text="Drug Name" val={drug} onChange={(e, val) => {
                    ...
                    if (val) {
                        ...
                        // arr[0].quantity = val.dosage * val.frequency_val * val.duration;
                        setRoute(val.route);
                        setParams(arr);
                        setDrugUom(val.uom)
                    } else {
            ...
            <div className={classes.medium}>
                <AddInput options={dosages} solo text="Dose" val={params[0].dosage} onChange={(e) => {
                    const arr = [...params];
                    arr[0].dosage = e.target.textContent ? e.target.textContent : e.target.value > 0 ? e.target.value : "";
                    setParams(arr);
                }} tail={drugUom} />
            </div>
            <div className={classes.medium}>
            ...
            <div className={classes.medium}>
                <AddTextInput
                    text="Quantity"
                    val={params[0].quantity}
                    onChange={(e) => {
                        const arr = [...params]
                        arr[0].quantity = e.target.value
                        setParams(arr)
                    }}
                />
            </div>
            <div className={classes.medium}>
                ...
                <Button className={classes.btn} style={{ color: '#06b6d4', background: '#e1fbff' }}
                    onClick={() => {
                        ...
                        if ((arr[index].drug === '' && index !== -1) || arr[index].params[0].quantity === undefined) {
                            arr.splice(index, 1);
                            setOpen(true);
                        }
                        setDrugList(arr);
                ...
                return <div className={classes.tbleFooter}>
                    ...
                    <div className={classes.medium}>
                        <AddInput options={dosages} solo text="Dose" val={params[pos].dosage} onChange={(e) => {
                            const arr = [...params];
                            arr[pos].dosage = e.target.textContent ? e.target.textContent : e.target.value > 0 ?e.target.value : 0;
                            setParams(arr);
                        }} tail={drugUom} />
                    </div>
                    <div className={classes.medium}>
                    ...
                    <div className={classes.medium}>
                        <AddTextInput
                            text="Quantity"
                            val={params[pos].quantity}
                            onChange={(e) => {
                                const arr = [...params]
                                if (e.target.value >= 0) {
                                    arr[pos].quantity = e.target.value
                                    setParams(arr)
                                }
