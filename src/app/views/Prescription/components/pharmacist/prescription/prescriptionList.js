...
const PrescriptionList = (props) => {
    ...
    useEffect(() => {
        if (prescriptions && prescriptions.length > 0) {
            setPrescription(prescriptions[0], 1); 
        }
    }, []);

    const TabItem = ({ pos }) => {
        if (prescriptions && prescriptions.length > pos) {
            return <div className={selectedIndex === pos ? classes.selectedTab : classes.tab}
                onClick={() => { setSelectedIndex(pos); setPrescription(prescriptions[pos], pos) }}>
                <h5 className={classes.header}>{prescriptions[pos].Clinic ? prescriptions[pos].Clinic.name : ''} - {dateParse(prescriptions[pos].createdAt)} - {prescriptions[pos].status}</h5>
