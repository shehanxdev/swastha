...
const RequestExchangeModal = ({ open, setOpen, setMsg }) => {
    ...
    const requestExchange = () => {
        ...
        for (let i = 0; i < warehouses.length; i++) {
            if (warehouses[i].qty > warehouses[i].total_quantity) {
                valid = false;
            }
            item_list.push({
        ...
        if (!valid) {
            setMsg("Issuing quantities should be less than stock quantity");
            setIsLoading(false);
            return;
        }
    ...
    return <Dialog open={open !== null}
        ...
        {open ? <div style={{ padding: '2rem' }}>
            ...
            <DialogContentText color='black'>
                ...
                <Grid container spacing={2} style={{ border: '1px solid #e0e0e0', marginBottom: '2em' }}>
                    <Grid item xs={4} style={{ borderRight: '1px solid #e0e0e0' }}>
                        <p style={{ fontSize: '0.8em' }}>Required Quantity : {(open.issuing - open.stock)}</p>
                    </Grid>
                    <Grid item xs={4} style={{ borderRight: '1px solid #e0e0e0' }}>
                        <p style={{ fontSize: '0.8em' }}>Quantity in Hand : {open.stock}</p>
                    </Grid>
                    <Grid item xs={4}>
                ...
                {warehouses.map((wh, pos) => <Grid container spacing={2} style={{ border: '1px solid #DFDFDF' }}>
                    ...
                    <Grid item xs={3}>
                        <TextField variant="outlined" type="number" size="small" placeholder={wh.qty} onChange={(e) => updateQty(e, pos)} />
                    </Grid>
                </Grid>)}
            ...
            <DialogActions>
                ...
                <Button size="small" variant="contained" style={{ background: '#00cbff', color: 'white' }} onClick={() => !isLoading ? requestExchange() : null} >{isLoading ? <CircularProgress size={15} color="primary" /> : "Request"}</Button>
            </DialogActions>
        </div> : null}
    </Dialog>
}
