...
import UpdateQunatity from './UpdateQuantity';
...
class OrderList extends Component {
            ...
            columns: [
                {
                    ...
                    options: {
                        ...
                        customBodyRender: (value, tableMeta, updateValue) => {
                            let data = this.state.data[tableMeta.rowIndex].order_quantity
                    ...
                    options: {
                        display: true,
                        customBodyRender: (value, tableMeta, updateValue) => {
                            let data = this.state.data[tableMeta.rowIndex].mystock_days
                            ...
                            return (
                                <>
                                    < IconButton
                                        ...
                                        <Icon color="primary">mode_edit_outline</Icon>
                                    </IconButton>
    ...
    async getCartItems() {
        ...
        let cartItems = this.state.getCartItems
        cartItems.warehouse_id = selected_warehouse_cache.id

        const { type } = this.props;
        if (type == 'SellsOrder') {
            cartItems.type = 'Sales Order'
        } 
        
        let res2 = await PharmacyOrderService.getOrderList(cartItems)
        if (res2.status) {
            if (res2.data.view.data.length != 0) {
                this.state.formData.order_requirement_id = res2.data.view.data[0].pharmacy_order_id
            }
            this.setState({
            ...
            <>
                ...
                <Dialog maxWidth="lg " open={this.state.qtyDialogView}
                    ...
                    <div className="w-full h-full px-5 py-5">
                        ...
                        <UpdateQunatity
                            id={this.state.update_item_id}
                            qty={this.state.update_item_qty}
                            view={this.state.qtyDialogView}
                            onSuccess={() => {
                                this.setState({
                                    qtyDialogView: false,
                                    tableDataLoaded: false,
...
export default withStyles(styleSheet)(OrderList)
