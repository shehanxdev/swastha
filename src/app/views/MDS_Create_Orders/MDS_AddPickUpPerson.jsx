...
class MDS_AddPickUpPerson extends Component {
    ...
    async loadData() {
        ...
        //let props = { type: ["Helper", 'Health Service Assistant', 'Driver'], owner_id: owner_id }
        let loadData = this.state.loadData;
        loadData.owner_id = owner_id
        ...
        let res = await PharmacyOrderService.getRemarks( {"type": "Order Remark", "remark": "Order Remark"})
        if (res.status == 200) {
    ...
    async componentDidMount() {
        ...
        order.order_exchange_id = this.props.id.type ? this.props.id.id: Object.values(this.props.id).map(val => val);
    ...
    async onSubmit() {
        ...
        if (this.state.order.order_exchange_id != null && this.state.order.delivery_mode != null) {
                                                                    ...
                                                                    <div style={{ display: 'flex', alignItems: 'flex-end' }}>
                                                                        <Grid item lg={1} md={1} sm={12} xs={12} style={{ display: 'flex', alignItems: 'center' }}>
                                                                            <LoonsButton
                                                                                ...
                                                                                progress={false}
                                                                                type="submit"
                                                                                disabled={this.state.order.pickup_person_id == null ? true : false}
                                                                                scrollToTop={true}
                                                                            // onClick={this.handleChange}
                                                                            >
                                                                                <span className="capitalize">Save</span>
