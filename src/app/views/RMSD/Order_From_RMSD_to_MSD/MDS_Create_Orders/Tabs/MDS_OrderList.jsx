...
class MDS_OrderList extends Component {
    ...
    async getCartItems() {
        this.setState({ tableDataLoaded: false })
        let res2 = await PharmacyOrderService.getOrderList(
            this.state.getCartItems
        )
        let formData = this.state.formData

        if (res2.status) {
            formData.order_requirement_id =
                res2.data.view.data[0]?.pharmacy_order_id
            this.setState(
                {
