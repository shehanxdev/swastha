...
import DetailLedgerReports from './DetailLedgerReport/index'
...
class CheckOtherStocks extends Component {
                            ...
                            return <Grid container spacing={1}>
                                ...
                                <Grid item>
                                    <Tooltip title="Stock Movement">
                                        <IconButton
                                            onClick={() => {
                                                let rowData = this.state.rows2[dataIndex]
                                                console.log('row data', this.state.rows2[dataIndex])
                                                window.location = `/drugbalancing/checkStock/detailedview/${this.state.rows2[dataIndex].items_id}?owner_id=${rowData.owner_id}&warehouse_id=${rowData.warehouse_id}`
    ...
    async loadTotalQTY() {
        ...
        if (batch_res.status == 201) {
            ...
            for (let i = 0; i < this.state.data.length; i++) {
                const totalqty = batch_res.data.posted.data.filter((x) => x.item_id == this.state.data[i].id
                );
    ...
    handleTabChange = (event, newValue) => {
        ...
        this.setState({ activeTab: newValue, suggestedWareHouses }, () => {
            console.log("suggestedWareHouses", suggestedWareHouses);
            this.suggestedWareHouse();
        });
    };
    ...
    async componentDidMount() {
        ...
        if (includesArrayElements(user.roles, ['Super Admin', 'ADMIN', 'RMSD ADMIN', 'RMSD OIC', 'RMSD MSA', 'RMSD Pharmacist', 'Drug Store Keeper', 'Blood Bank MLT (NOIC)', 'Medical Laboratory Technologist', 'Radiographer', 'Chief MLT', 'Chief Radiographer', 'RMSD Distribution Officer'])) {
    ...
    // get warehouse for other institute
    // async getOtherWarehouseInfo() {
    //     let params = {
    //         // owner_id: '000',
    //         store_type:'drug_store',
    //         other_owner_id:this.state.
    //     }
    //     let res = await WarehouseServices.getWareHouseUsers(params);
    ...
    // dropDownReset(type, id) {

    //     let suggestedWareHouses = this.state.suggestedWareHouses
    //         // suggestedWareHouses.warehouse_id = id
    ...
    async loadWarehousesForExchange() { //load my warehouses to select in order exchange
        // this.setState({ loaded: false })
        var user = await localStorageService.getItem('userInfo');
        var id = user.id;
        var all_pharmacy_dummy = [];
                    ...
                    <LoonsCard>
                        ...
                        <div
                            ...
                            <Grid
                                ...
                                <Grid item="item">
                                    <Typography variant="h6" className="font-semibold">All Items</Typography>
                                </Grid>
                                <Grid item="item">
                                    ...
                                    <RadioGroup row="row" defaultValue="order">
                                        <div style={{ display: 'flex', marginRight: '14px', alignItems: 'center' }}>
                                            <Grid
                                                className='pt-1 pr-3'
                                            >
                                                <Typography>{this.state.selectWarehouseViewName !== null ? "You're in " + this.state.selectWarehouseViewName : null}</Typography>
                                            </Grid>
                                            {/*     <LoonsButton
                                                color='primary'
                                                onClick={() => {
                                    ...
                                    <Autocomplete
                                        ...
                                        onChange={(e, value) => {
                                            if (value != null) {
                                                let formData =
                                                    this.state.formData
                                                    console.log('abc',formData.long_description)
                                                formData.long_description = value.long_description
                                                this.setState({ formData })
                                            }
                                            else {
                                                let formData =
                                                    this.state.formData
                                                formData.long_description = null
                                        ...
                                        value={this.state.allVENS.find(
                                            ...
                                            (obj) =>
                                                obj.id ==
                                                this.state.formData.long_description
                                        )}
...
'this field is required',
                                ...
                                >
                                    ...
                                    <Autocomplete
                                        disableClearable
                                        className="w-full"
                                        options={this.state.allGroups.filter((ele) => ele.status == 'Active').sort((a, b) => a.code?.localeCompare(b.code))
                                        }
                                        onChange={(e, value) => {
                                            if (value != null) {
                                                let formData =
                                ...
                                >
                                    ...
                                    <Autocomplete
                                        disableClearable
                                        className="w-full"
                                        options={this.state.allSerials.filter((ele) => ele.status == 'Active').sort((a, b) => a.name?.localeCompare(b.name))}
                                        onChange={(e, value) => {
                                            if (value != null) {
                                                let formData =
                                                    this.state.formData
                                ...
                                >
                                    ...
                                    <Autocomplete
                                        disableClearable
                                        className="w-full"
                                        options={this.state.allWH.sort((a, b) => (a.name?.localeCompare(b.name)))}
                                ...
                                >
                                    ...
                                    <Autocomplete
                                        disableClearable
                                        className="w-full"
                                        options={this.state.allStorages.filter((ele) => ele.status == 'Active').sort((a, b) => (a.name?.localeCompare(b.name)))}
                                        onChange={(e, value) => {
                                            if (value != null) {
                                                let formData =
                                                    this.state.formData
                                ...
                                >
                                    ...
                                    <Autocomplete
                                        ...
                                        options={this.state.allUOMS.filter(
                                            (ele) => ele.status == 'Active').sort((a, b) => (a.name?.localeCompare(b.name)))}
                                        onChange={(e, value) => {
                                            if (value != null) {
                                                let formData =
                                                    this.state.formData
                                                            ...
                                                            <Grid container spacing={2}>
                                                                ...
                                                                >
                                                                    ...
                                                                    <Autocomplete
                                                                        ...
                                                                        // disableClearable
                                                                        className="w-full"
                                                                        options={this.state.wardList.sort((a, b) => a.store_id?.localeCompare(b.store_id))}
                                                                        getOptionLabel={(option) => `${option.store_id} / ${option.name}`}
                                                                        onChange={(e, value) => {
                                                                            this.setState({
                                                                                resetFilters: false,
                                                                            })
                                                                            ...
                                                                            if (value != null) {
                                                                                ...
                                                                                let suggestedWareHouses = this.state.suggestedWareHouses;
                                                                                suggestedWareHouses.warehouse_id = null;
                                                                        ...
                                                                        value={
                                                                            ...
                                                                            this.state.wardList.find(
                                                                                (obj) => obj.id == this.state.suggestedWareHouses.warehouse_id
                                                                            )
                                                                            // : null
                                                                        }
                                                                        renderInput={(params) => (
                                                                            <TextValidator
                                                                                ...
                                                                                onChange={(e) => {
                                                                                    ...
                                                                                    this.getWard(e.target.value);
                                                                                    // }
                                                                                }}
                                                                                value={this.state.suggestedWareHouses.warehouse_id}
                                                                            />
                                                                        )}
                                                                    />
                                                                </Grid>
                                                                ...
                                                                >
                                                                    ...
                                                                    <Autocomplete
                                                                        // disableClearable
                                                                        className="w-full"
                                                                        options={this.state.pharmacyList.sort((a, b) => a.store_id?.localeCompare(b.store_id))}
                                                                        getOptionLabel={(option) =>
                                                                        ...
                                                                        renderInput={(params) => (
                                                                            <TextValidator {...params}
                                                                                ...
                                                                                onChange={(e) => {
                                                                                    // if (e.target.value.length > 3) {
                                                                                    this.getPharmacy(e.target.value)
                                                                                    // }
                                                                                }}
                                                                                value={this.state.suggestedWareHouses.warehouse_id}
                                                                ...
                                                                >
                                                                    ...
                                                                    <Autocomplete
                                                                        // disableClearable
                                                                        className="w-full"
                                                                        options={this.state.ClinicList.sort((a, b) => a.store_id?.localeCompare(b.store_id))}
                                                                        getOptionLabel={(option) =>
                                                                            `${option.store_id} / ${option.name}`
                                                                        }
                                                                ...
                                                                >
                                                                    ...
                                                                    <Autocomplete
                                                                        // disableClearable
                                                                        className="w-full"
                                                                        options={this.state.drugStoreList.sort((a, b) => a.store_id?.localeCompare(b.store_id))}
                                                                        getOptionLabel={(option) =>
                                                                            `${option.store_id} / ${option.name}`
                                                                        }
                                                                        ...
                                                                        renderInput={(params) => (
                                                                            <TextValidator {...params}
                                                                                ...
                                                                                onChange={(e) => {
                                                                                    // if (e.target.value.length > 3) {
                                                                                    this.getDrugStore(e.target.value)
                                                                                    // }
                                                                                }}
                                                                                value={this.state.suggestedWareHouses.warehouse_id}
                            ...
                            <Autocomplete
                                ...
                                onChange={(e, value) => {
                                    if (value != null) {
                                        ...
                                        let suggestedWareHouses = this.state.suggestedWareHouses
                                        suggestedWareHouses.warehouse_id = value.id
                                        this.setState({ selectWarehouseView: false, suggestedWareHouses, selectWarehouseViewName: value.name })
                                        //this.suggestedWareHouse()
