...
import { dateParse, dateTimeParse, includesArrayElements } from "utils";
import { styles } from "@material-ui/pickers/views/Calendar/Calendar";
import ClinicService from "app/services/ClinicService";
import PharmacyService from "app/services/PharmacyService";

class MSD_MSA_ToBeAllocated extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            formData: {
                ...
                from_date: null,
                to_date: null,
                status: ['ORDERED', 'Pending', 'APPROVED', 'ALLOCATED'],
                date_type: null,
                from: null,
                search: null,
                from_owner_id:null
                ...
                , {
                    ...
                    options: {
                        customBodyRenderLite: (dataIndex) => {
                            if (this.state.data) {
                                let data = this.state.data[dataIndex]?.approved_date;
                                if (data) {
                            ...
                            if (this.state.data) {
                                ...
                                if (this.state.data[dataIndex]?.Delivery == null) {
                                    ...
                                    if (this.state.data[dataIndex]?.Delivery?.time_from != null) {
                                        from = this.state.data[dataIndex]?.Delivery?.time_from
                                    }

                                    if (this.state.data[dataIndex]?.Delivery?.time_to != null) {
                                        to = this.state.data[dataIndex]?.Delivery?.time_to
                                    }
                        ...
                        customBodyRender: (value, tableMeta, updateValue) => {
                            return (<> < IconButton className="text-black" onClick={
                                ...
                                <IconButton
                                    ...
                                    onClick={() => window.location = `/msa_all_order/all-orders/order/${this.state.data[tableMeta.rowIndex]?.id
                                        }/${this.state.data[tableMeta.rowIndex]?.number_of_items
                                        }/${this.state.data[tableMeta.rowIndex]?.order_id
                                        }/${this
                                            .state
                                            .data[tableMeta.rowIndex]?.Employee?.name
                                        }/${this
                                            .state
                                            .data[tableMeta.rowIndex]?.Employee?.contact_no
                                        }/${this
                                            .state
                                            .data[tableMeta.rowIndex]?.status
                                        }/${this
                                            .state
                                            .data[tableMeta.rowIndex]?.type
            ...
            <MainContainer>
                <Grid container="container" spacing={2}>
                    <Grid item="item" xs={12}>
                        <Typography variant="h5" className="font-semibold">Pending Orders</Typography>
                        <Divider />
                    </Grid>
                </Grid>
                {/* {this.state.loaded ?
...
export default MSD_MSA_ToBeAllocated