...
class MSD_Print extends Component {
    ...
    async loadInstitute(owner_id) {
        ...
        console.log('chack hospital', this.state.hospital)
    }

    async sendPrinted() {
        let status = this.state.data2.stv_printed
        let formData = {
            stv_printed: true
        }
        //let res = await ChiefPharmacistServices.addDistributionRemark(formData,  this.props.order_exchange_id)
        if (status != "true") {
            let res = await ChiefPharmacistServices.addDistributionRemark(formData, this.props.order_exchange_id)
                            ...
                            <table className="w-full">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        >                                                                    {
                                                this.state.data2.Delivery?.Employee?.name ? this.state.data2.Delivery?.Employee?.name :
                                                    this.state.data2?.Employee?.name
                                    ...
                                    <tr>
                                        ...
                                        >                                                                    {
                                                this.state.data2?.Delivery?.Employee?.designation ? (this.state.data2?.Delivery?.Employee?.designation=="Drug Store Keeper"?"Drug Store Pharmacist":this.state.data2?.Delivery?.Employee?.designation) :
                                                    (this.state.data2?.Employee?.designation=="Drug Store Keeper"?"Drug Store Pharmacist":this.state.data2?.Employee?.designation)
                                            }</td>
                                    </tr>
                                    <tr>
                                        <td
                                        ...
                                        >                                                                    {
                                                this.state.data2.Delivery?.Employee?this.state.data2.Delivery?.Employee.nic:
                                                this.state.data2?.Employee.nic
                                            }</td>
                                    </tr>
                                </tbody>
                            </table>
                        ...
                        <div className="bg-white px-5" ref={(el) => (this.componentRef = el)} >
...

                            {
                                this.state.items?.map((item, index) => {
                                    ...
                                    let isIncluedServiceCharge = false
                                        ...
                                        <div className="w-full" >
                                            ...
                                            {/* Starting Batches section */}
                                            {
                                                filterdData?.map((drug, batchIndex) => {
                                                    ...
                                                    if (drug.OrderItem?.OrderExchange?.type === 'CASH SALES' || drug.OrderItem?.OrderExchange?.type === 'Sales Order') {
                                                        isIncluedServiceCharge = true
                                                        // serviceCharges = msdServiceChargesCal(sub_total)
                                                    }
                                                    console.log('checking data', drug.OrderItem?.OrderExchange?.type)
                                                    ...
                                                    if (isIncluedServiceCharge) {
                                                        ...
                                                        unitPrice = roundDecimal(drug?.ItemSnapBatchBin?.ItemSnapBatch?.unit_price, 3)
                                                        totalPrice = parseFloat(drug?.allocated_quantity) * parseFloat(unitPrice)
                                                        netSubTotal = netSubTotal + parseFloat(totalPrice)
                                                    }
                                                        ...
                                                        <div className={newPage ? "w-full" : "w-full"}>
                                                                ...
                                                                <div className="w-full">
                                                                    ...
                                                                    <div class="flex ">
                                                                        ...
                                                                        <div
                                                                            ...
                                                                            style={{ width: '40%' }}
                                                                        >
                                                                            <SubTitle title={this.state.data2.stv_printed == "true" ? "Accountant Copy (Duplicate)" : "Accountant Copy (Original)"}></SubTitle>
                                                                        </div>
                                                                    </div>
                                                                            ...
                                                                            >
                                                                                <div className="flex">
                                                                                    ...
                                                                                    <p className="text-12 l my-0 ml-2 ">
                                                                                        {
                                                                                            this.state.selected_warehouse_cache?.name 
                                                                                        }
                                                                                ...
                                                                                <div className="flex">
                                                                                    <p className="text-12 l my-0">
                                                                                        Receiving Warehouse :{' '}
                                                                                        {
                                                                                            this.state.data2?.fromStore?.name+ (this.state.hospital?.Department?.name ? ' (' + this.state.hospital.Department.name + ')' : ' ')
                                                                                        }
                                                                                    </p>
                                                                        ...
                                                                        <div className="w-full">
                                                                            ...
                                                                            >
                                                                                ...
                                                                                <div className="flex">
                                                                                    ...
                                                                                    <p className="text-12 l my-0 ml-2 ">
                                                                                        {/*  {dateParse(new Date())} :{' '} */}
                                                                                        {dateTimeParse(this.state.data2.issued_date)}
                                                                                    </p>
                                                                                </div>
                                                                                <div className="flex">
                                                                                    <p className="text-12 l my-0">
                                                            ...
                                                            <div className='w-full' >
                                                                ...
                                                                <div className="row px-3 ">
                                                                    ...
                                                                    {console.log('cheking datatatatat', drug?.OrderItem?.ItemSnap)}
                                                                    <div className="cell" style={{ width: '15%' }}>
                                                                        {(drug?.OrderItem?.ItemSnap?.converted_order_uom === "EU") ? convertTocommaSeparated(drug?.allocated_quantity * drug?.OrderItem?.ItemSnap?.item_unit_size, 2) + ' ' + drug?.OrderItem?.ItemSnap?.DisplayUnit?.name + ' (' + drug?.allocated_quantity + ' ' + drug?.OrderItem?.ItemSnap?.MeasuringUnit?.name + ' )'   : convertTocommaSeparated(drug?.allocated_quantity, 2)}
                                                                    </div>
                                                                    <div className="cell" style={{ width: '15%' }}>
 ...
 */}
                                                                    ...
                                                                    <div className="cell" style={{ width: '15%' }}>
                                                                        {/* {convertTocommaSeparated(drug?.ItemSnapBatchBin?.ItemSnapBatch?.unit_price, 3)} */}
                                                                        {convertTocommaSeparated(unitPrice, 3)}
                                                                    </div>
                                                                    <div className="cell" style={{ width: '15%' }}>
                                                                        {/* {convertTocommaSeparated(parseFloat(drug?.allocated_quantity) * parseFloat(roundDecimal(drug?.ItemSnapBatchBin?.ItemSnapBatch?.unit_price, 3)), 3)} */}
                                                                ...
                                                                <div className='w-full'>
                                                                    {/* <table className='mt-5 ml-3'>
                                                                        <tr>
                                                                            ...
                                                                            <td className="">
                                                                                {convertTocommaSeparated(sub_total,2)}
                                                                            </td>
                                                                        </tr>
                                                                    </table> */}

                                                                    <table className='w-full ml-3 mt-3'>
                                                                        {/*  <tr>
                                                                            <td className=" font-bold">Total Value(LKR) :{' '}</td>
                                                                            <td>{convertTocommaSeparated(netSubTotal || 0, 3)}</td>
                                                                        </tr>
                                                                        <tr>
                                                                            <td className=" font-bold">Services Charges(LKR) :{' '}</td>
                                                                            <td>{convertTocommaSeparated(serviceCharges || 0, 3)}</td>
                                                                        </tr> */}
                                                                        <tr>
                                                                            {/* <td className=" font-bold">Final Value(LKR) :{' '}</td> */}
                                                                            <td className=" font-bold">Total Value(LKR) :{' '}</td>
                                                                            <td>{convertTocommaSeparated(netSubTotal, 2)}</td>
                                                                        </tr>
                                                                    </table>
