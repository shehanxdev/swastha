...
class OrderTab1 extends Component {
        ...
        this.state = {
            ...
            columns: [
                ...
                },
                {
                    ...
                    options: {
                        customBodyRender: (value, tableMeta, updateValue) => {
                            console.log(this.state.data, tableMeta.rowIndex, "MSDRequirement>>>>")
                            return (
                                <LoonsButton onClick={() => this.props.history.push(`/ordering/forecast/${value}/${this.state.data.length ? this.state.data[tableMeta.rowIndex].item_id : null}`)} color="primary">Order</LoonsButton>
                                ...
                                <TextField
                                    ...
                                    onChange={(e) => {
                                        ...
                                        if ((parseInt(e.target.value) <= parseInt(this.state.rows2[dataIndex].total_quantity)) || e.target.value == null || e.target.value == '') {
                                            this.addSuggestedWareHouseCart(this.state.rows2[dataIndex].warehouse_id, e.target.value, this.state.rows2[dataIndex].warehouse_name, this.state.rows2[dataIndex].total_quantity, 10)

                                        } else {
                                            this.addSuggestedWareHouseCart(this.state.rows2[dataIndex].warehouse_id, this.state.rows2[dataIndex].total_quantity, this.state.rows2[dataIndex].warehouse_name, this.state.rows2[dataIndex].row.total_quantity, 10)
                                            this.setState({
                                                alert: true,
            ...
            loading: false,
            Allversions: [],
            allStatus: ['Active', 'Pending Approval', 'HSCO Checked', 'AD Forecast Approved', 'Ordered', 'HSCO Order Ammended', 'Distribution Officer Order Checked', 'HSCO Ammended', 'Started', 'Order', 'AD Order Approved', 'Accountant Approved', 'AD Forecast Rejected', 'AD Checked', 'HSCO Approved', 'HSCO Rejected', 'Accountant Rejected'],
            formData: {
                ven_id: null,
                class_id: null,
                category_id: null,
    ...
    async loadOrderList() {
        ...
        formData.created_by = localStorageService.getItem("userInfo").id
        formData.type = this.state.activeType
        formData.pending_only = formData.status == null ? true : null
    ...
    async setTab(newValue) {
        ...
        if (newValue == 0) {
            this.setState({ activeType: 'Msd Order', activeTypeTab: 0 }, () => { this.loadOrderList() })
        } else {
            this.setState({ activeType: 'MSD ALL ITEMS', activeTypeTab: 1 }, () => { this.loadOrderList() })
        }
    }
    ...
    async generateOrder() {
        ...
        let item_ids = this.state.selectedItems.map((x) => x.id)

        if (this.state.activeType == "MSD ALL ITEMS") {
            payload.type = "MSD ALL ITEMS"
        ...
        return (<Fragment>
            <MainContainer>
                <LoonsCard>
                    ...
                    <Tabs
                        value={this.state.activeTypeTab}
                                ...
                                <Grid container="container" spacing={2}>
                                    ...
                                    <Grid
                                        ...
                                        <TextValidator className='' placeholder="Search"
                                            ...
                                            InputProps={{
                                                endAdornment: (
                                                    <InputAdornment position="end" onClick={() => { this.loadOrderList() }}>
                                    ...
                                    <Grid
                                        ...
                                        xs={12}>
                                        {/* Submit Button */}
                                        <LoonsButton className=" mr-2" style={{marginTop:26}} progress={false} type='submit'
                                    ...
                                    {this.state.activeTypeTab == 1 && <div className='w-full mt-5'>
                                        <Divider />
                                        <Grid container className='w-full' spacing={1}>
                                            <Grid item lg={3} md={3} xs={12}>
                                                ...
                                                <Autocomplete
                                                    ...
                                                    onChange={(e, value) => {
                                                        if (value != null) {
                                                            this.setState({ selectedTempItem: value })
                                            ...
                                            <Grid item lg={3} md={3} xs={12}>
                                                <LoonsButton style={{ marginTop: '26px' }}
                                                    onClick={() => {
                                                        let selectedItems = this.state.selectedItems
                                                        if (selectedItems.length > 0) {
                                                            let findIndex = selectedItems.findIndex(this.state.selectedTempItem)
                                                            console.log("item index", findIndex)
                                                            if (findIndex == -1) {
                                                                selectedItems.push(this.state.selectedTempItem)
                                        ...
                                        <Grid>
                                            <LoonsTable
                                                //title={"All Aptitute Tests"}
                                                id={'selectedItems'} data={this.state.selectedItems}
                                                columns={this.state.selecteditemsColumns}
                                                options={{
                                                    pagination: true,
                                                    serverSide: false,
                                                    print: false,
                                        ...
                                        <ValidatorForm
                                            ...
                                            <Grid
                                                ...
                                                <Grid item="item" lg={4} md={4} xs={12} className="mr-2">
                                                    ...
                                                    <LoonsButton color="primary" size="medium" type="submit">Generate Order</LoonsButton>
