...
class DelailsView extends Component {
    ...
    async loadDebitNoteApprovalData(note_no) {
        ...
        if (res.status === 200) {
            ...
            console.log("----------------------------------------------------")
            console.log("Approved Data", filterData2)
    ...
    async generateOriginal() {
        ...
        console.log("Sended GRN data -->>", data)
                                            ...
                                            <LoonsCard>
                                                                ...
                                                                >
                                                                    <div style={{ display: "flex", flexDirection: "row", alignItems: "center", height: "40px" }}>
                                                                        ...
                                                                        <div style={{ flex: 1 }} className='mx-2'>
                                                                            <AddNumberInput
                                                                                disabled={['Service Charge', 'Service VAT'].includes(item.TransactionType?.type) && editOptionEnable
                                                                                ...
                                                                                onChange={(e) => {
                                                                                    ...
                                                                                    if (item.TransactionType?.type == "Service Charge") {
                                                                                        data.DebitNoteCharges.filter(e => e.TransactionType?.type == "Service Charge")[0].percentage_or_value = e.target.value

                                                                                        let cal = (this.state.data?.Consignment?.values_in_lkr * e.target.value) / 100
                                                                                        let vat_val = data.DebitNoteCharges.filter(e => e.TransactionType?.type == "Service VAT")[0].percentage_or_value
                                                                                        let cal2 = parseFloat(cal * vat_val / 100)

                                                                                        data.DebitNoteCharges.filter(e => e.TransactionType?.type == "Service Charge")[0].amount = cal
                                                                                        data.DebitNoteCharges.filter(e => e.TransactionType?.type == "Service VAT")[0].amount = cal2

                                                                                        let chargesAmount = this.state.data?.DebitNoteCharges.filter(e =>
                                                                                            e.TransactionType?.type !== "Service Charge" &&
                                                                                            e.TransactionType?.type !== "Service VAT"
                                                                                        ).reduce((sum, transaction) => sum + parseFloat(transaction.amount), 0) +
                                                                                            parseFloat(cal) +
                                                                                            parseFloat(cal2);

                                                                                        data.final_value = parseFloat(this.state.data?.Consignment?.values_in_lkr) + parseFloat(chargesAmount)
                                                                                        data.total_charges = chargesAmount



                                                                                    }

                                                                                    if (item.TransactionType?.type == "Service VAT") {
                                                                                        data.DebitNoteCharges.filter(e => e.TransactionType?.type == "Service VAT")[0].percentage_or_value = e.target.value

                                                                                        let cal = (data.DebitNoteCharges.filter(e => e.TransactionType?.type == "Service Charge")[0].amount * e.target.value) / 100

                                                                                        data.DebitNoteCharges.filter(e => e.TransactionType?.type == "Service VAT")[0].amount = cal

                                                                                        let chargesAmnt = this.state.data?.DebitNoteCharges.filter(e =>
                                                                                            e.TransactionType?.type !== "Service VAT"
                                                                                        ).reduce((sum, transaction) => sum + parseFloat(transaction.amount), 0) +
                                                                                            parseFloat(cal)


                                                                                        data.final_value = parseFloat(this.state.data?.Consignment?.values_in_lkr) + parseFloat(chargesAmnt)
                                                                                        data.total_charges = chargesAmnt
                                                                        ...
                                                                        <div style={{ flex: 1 }} className='mx-2'>
                                                                            <TextValidator
                                                                                disabled={(this.state.userRoles.includes("SPC MA") &&
                                                                                    this.state.data?.status == "Pending" ||
                                                                                    this.state.data?.status == "REJECTED" ||
                                                                                    this.state.data?.status == "REINSTATE" ||
                                                                                    this.state.data?.status == "RESUBMITTED"
                                                                                ) ||
                                                                                    (this.state.userRoles.includes("SPC Supervisor") &&
                                                                                        (this.state.data?.status == "SUPERVISOR APPROVED" ||
                                                                                            this.state.data?.status == "Pending")
                                                                                    )

                                                                                    ||
                                                                                    (this.state.userRoles.includes("SPC Manager") &&
                                                                                        (this.state.data?.status == "SUPERVISOR APPROVED" ||
                                                                                            this.state.data?.status == "APPROVED")) ? false : true}
                                                                                size="small"
                                                                                ...
                                                                                onChange={(e) => {
                                                                                    ...
                                                                                    if (item.TransactionType?.type == "Additional Charges") {
                                                                                        if (!isNaN(e.target.value) && e.target.value !== null && e.target.value !== "") {


                                                                                            data.DebitNoteCharges.filter(e => e.TransactionType?.type == "Additional Charges")[0].amount = parseFloat(e.target.value)

                                                                                            let chargesAmnt = this.state.data?.DebitNoteCharges.filter(e =>
                                                                                                e.TransactionType?.type !== "Additional Charges"
                                                                                            ).reduce((sum, transaction) => sum + parseFloat(transaction.amount), 0) +
                                                                                            ...
                                                                                            data.DebitNoteCharges.filter(e => e.TransactionType?.type == "Additional Charges")[0].amount = parseFloat(0.00)
                                                                                            let chargesAmnt = this.state.data?.DebitNoteCharges.filter(e =>
                                                                                                e.TransactionType?.type !== "Additional Charges"
                                                                                            ).reduce((sum, transaction) => sum + parseFloat(transaction.amount), 0) +
                                                                                                parseFloat(0.00)
                                                ...
                                                <Grid container="container" spacing={2}>
                                                    <Grid container spacing={2}
                                                        ...
                                                        {(this.state.data?.status === 'REINSTATE') && this.state.userRoles.includes("SPC MA") ?
                                                            <Button
                                                                ...
                                                                startIcon={<SaveIcon />}
                                                                onClick={() => this.setState({ updateAdditionalChargeOpen: true })}
                                                            >
                                                        ...
                                                        {((((this.state.data?.status === 'Pending' || this.state.data?.status === 'RESUBMITTED') && (this.state.userRoles.includes("SPC Supervisor") || this.state.userRoles.includes("SPC MA"))) ||
                                                            ...
                                                            <Button
                                                                ...
                                                                onClick={() => this.setState({ updateDebitNoteOpen: true })}
                                                            >
                                                                <span className="capitalize">
                                                                    Save
                                                                </span>
...

                                                        {
                                                            ...
                                                            <Button
                                                                ...
                                                                onClick={() => {
                                                                    ...
                                                                    } else if (this.state.grnDetails.every(obj => obj.status === "APPROVED PARTIALLY COMPLETED")) {
