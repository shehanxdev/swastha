...
export default function PODetails({ order_no }) {
                ...
                customBodyRenderLite: (dataIndex) => {
                    ...
                    return (
                        <Grid className="px-2 flex justify-start">
                            ...
                            <Typography>{dateParse(tempTableData.createdAt)}</Typography>
                        </Grid>
                    )
                },
            },
                ...
                customBodyRenderLite: (dataIndex) => {
                    ...
                    return (
                        <Grid className="px-2 flex justify-start">
                            ...
                            <Typography>{convertTocommaSeparated(tempTableData?.OrderListItem.quantity ?? "0", 0)}</Typography>
                        </Grid>
                    )
                },
            },
    ...
    const loadData = async () => {
        ...
        let updatedData = { ...itemfilterData, order_no: order_no }

        setDataIsLoading(true)
        let res = await SPCServices.getAllPurchaseOrders(updatedData)
    ...
    async function LoadItemDetails(po_id) {
        ...
        if (result.status == 200) {
            ...
            const subTot = result.data.view.data.reduce((accumulator, currentValue) => {
                return accumulator + parseInt(currentValue.total ?? 0, 10);
            }, 0)

            const totTax = result.data.view.data.reduce((accumulator, currentValue) => {
                return accumulator + parseInt(currentValue.tax_amount ?? 0, 10);
            }, 0)

            const totDis = result.data.view.data.reduce((accumulator, currentValue) => {
                return accumulator + parseInt(currentValue.discount ?? 0, 10);
            }, 0)
            ...
            setFormData({
                ...
                total_payable: result.data.view.data.total_payable ?? 0
    ...
    function setData(data) {
        ...
        setItemFormData({
            ...
            unit: data?.unit,
            price: data?.price,
            total: (parseInt(currentItemDetails?.OrderListItem.quantity || 0) - parseInt(currentItemDetails?.OrderListItem.allocated_quantity || 0)),
    ...
    return (
        <Fragment className="p-2">
                                    ...
                                    <Grid item xs={12} md={4}>
                                        <TextValidator
                                            ...
                                            onChange={(e) => {
                                                ...
                                                setFormData({ ...formData, handl_and_packaging_charge: e.target.value, final_tot: (formData?.grand_total + subTot) })
                                            }}
                                            InputLabelProps={{
                                    ...
                                    <Grid item xs={12} md={4}>
                                        <TextValidator
                                            ...
                                            onChange={(e) => {
                                                ...
                                                setFormData({ ...formData, freight_chargers: e.target.value, final_tot: (formData?.grand_total + subTot) })
                                            }}
                                            InputLabelProps={{
                                    ...
                                    <Grid item xs={12} md={4}>
                                        <TextValidator
                                            ...
                                            onChange={(e) => {
                                                ...
                                                setFormData({ ...formData, other_charge: e.target.value, final_tot: (formData?.grand_total + subTot) })
                                            }}
                                            InputLabelProps={{
            ...
            <Dialog maxWidth="md" fullWidth open={ItemOpen} onClose={() => { }} aria-labelledby="form-dialog-title">
                            ...
                            <Grid container spacing={2}>
                                ...
                                <Grid item xs={12} md={4}>
                                    <Grid container spacing={2}>
                                        ...
                                        <Grid item xs={12} className='flex justify-start'>
                                            <Typography> {convertTocommaSeparated(currentItemDetails?.OrderListItem.quantity ?? '0', 0)}</Typography>
                                        </Grid>
                                    </Grid>
                                ...
                                <Grid item xs={12} md={4}>
                                    <Grid container spacing={2}>
                                        ...
                                        <Grid item xs={12} className='flex justify-start'>
                                            <Typography>  {convertTocommaSeparated(currentItemDetails?.OrderListItem.allocated_quantity ?? '0', 0)}</Typography>
                                        </Grid>
                                    </Grid>
                                        ...
                                        <Grid item xs={12} md={4}>
                                            <Grid container spacing={2}>
                                                ...
                                                <Grid item xs={12} md={6} className='flex justify-start'>
                                                    <Typography className='text-15 font-bold'> {convertTocommaSeparated((itemFormData?.total * (itemFormData?.price / itemFormData?.unit)), 1, 4)}</Typography>
                                                </Grid>
                                                ...
                                                <Grid container spacing={1}>
                                                    ...
                                                    <Grid item xs={12}>
                                                        <TextValidator
                                                            ...
                                                            onChange={(e) => {
                                                                if (selectedValue == "Value") {
                                                                    setItemFormData({ ...itemFormData, discount: e.target.value })
                                                                    setDiscount(e.target.value)
                                                                } else {
                                                                    setItemFormData({ ...itemFormData, discount_precentage: e.target.value })

                                                                    let dis = (itemFormData?.total * itemFormData?.price) * (e.target.value / 100)
                                                                    setDiscount(dis)
                                                                }
                                                            }}
                                                    ...
                                                    <Grid item xs={12}>
                                                        <Grid container spacing={2}>
                                                            ...
                                                            <Grid item xs={12} md={6} className='flex justify-end'>
                                                                <Typography className='text-15 font-bold'> {convertTocommaSeparated(discount, 1, 4)}</Typography>
                                                            </Grid>
                ...
                <DialogActions>
                    ...
                    <Button
                        ...
                        variant='contained' size='large' onClick={() => { }} color="primary">
                        Update
                    </Button>
                </DialogActions>
            </Dialog>
            ...
            <Dialog
                ...
                <div className="w-full h-full px-4 pb-5">
                    <ViewPODetails pageData={selected_data} />
                </div>
            </Dialog>
