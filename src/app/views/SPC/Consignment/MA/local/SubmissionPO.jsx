...
class SubmissionPO extends Component {
    ...
    onSubmit = async (status, remark, data) => {
        ...
        let res = await SPCServices.changeConsignmentApproval(data?.id, params)

        if (res.status === 200 && consignment_res.status === 200) {
            this.setState({
                alert: true,
            ...
            <Fragment>
                ...
                <Dialog
                    ...
                    <div className="w-full h-full px-4">
                        {/* {this.state.sequence === 0 ? */}
                        <LocalDetails isEdit={this.state.userRoles.includes('SPC MA')} isResubmit={false} data={this.state.single_data} id={this.state.selected_id} isApprove={this.state.isApprove} handleClose={() => this.setState({ approveOpen: false })} onSubmit={(status, remark, data) => { this.onSubmit(status, remark, data) }} handleReject={() => this.setState({ rejectOpen: true })} />
