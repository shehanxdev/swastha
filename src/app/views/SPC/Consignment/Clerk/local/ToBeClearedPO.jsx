...
import LocalShipment from "./LocalShipment"
...
class ApprovalPO extends Component {
    ...
    onSubmit = async () => {
        ...
        const { delivery_details } = this.state.storedData
        ...
        try {
            let container_res = await SPCServices.createConsignmentContainer(updatedDetails)
            if (container_res.status === 201) {
                ...
                if (consignment_res.status) {
                    this.setState({
                        severity: "success",
                        alert: true,
                        message: "SPC Consignment Delivery Details was Updated",
                        forwardOpen: false,
                        isForward: true
                    })
                } else {
                ...
                this.setState({
                    severity: "error",
                    alert: true,
                    message: "SPC Consignment Delivery Details Updation was Unsuccessfull"
                })
            }
        } catch (error) {
            this.setState({
            ...
            <Fragment>
                ...
                <Dialog
                    ...
                    <div className="w-full h-full px-4 pb-5">
                        {/* {this.state.sequence === 0 ? */}
                        <LocalShipment data={this.state.single_data} isEdit={true} storeData={this.handleDataStore} handleClose={() => this.setState({ approveOpen: false })} submitOpen={() => this.setState({ forwardOpen: true })} handleReject={() => this.setState({ rejectOpen: true })} />
                        {/* } */}
                    </div>
                </Dialog>
                <ConfirmationDialog
                    text="Are you Sure to forward the LDCN?"
