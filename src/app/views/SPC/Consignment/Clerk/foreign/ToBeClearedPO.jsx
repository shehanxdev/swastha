...
import ForeignShipment from './ForeignShipment'
...
class DraftPO extends Component {
                                    ...
                                    <Tooltip title="View WDN">
                                        <IconButton
                                            ...
                                            onClick={() => {
                                                ...
                                                this.setState({ approveOpen: true, selected_id: id, selected_data: data }, () => {
    ...
    onSubmit = async () => {
        ...
        try {
            ...
            const container_res = await this.updateContainerDetails();

            if (container_res.status === 201 || vessel_res.status === 200 || vessel_res.status === 201 || container_res.status === 200) {
                let consignment_res = await SPCServices.changeConsignmentByID(this.state.selected_id, { status: "DELIVERED" })
                if (consignment_res.status) {
                    this.setState({
                        severity: "success",
                        alert: true,
                        message: "SPC Consignment Delivery Details and Vessel Details was Updated",
                        forwardOpen: false,
                        isForward: true
                    }, () => {
                        this.loadData()
                ...
                this.setState({
                    severity: "error",
                    alert: true,
                    message: "SPC Consignment Delivery Details and Vessel Details Updation was Unsuccessfull"
                })
            }
        } catch (error) {
            this.setState({
    ...
    updateVesselDetails = async () => {
        const { vessel_details, ConsigmentVesselData } = this.state.storedData
        const vesselDetails = {
            ...vessel_details?.[0],
    ...
    updateContainerDetails = async () => {
        ...
        const { delivery_details, ConsignmentContainers } = this.state.storedData
        const deliveryDetails = {
            ...delivery_details?.[0],
            warf_clerk_id: user_id,
            consignment_id: this.state.selected_id,
        };
            ...
            <Fragment>
                ...
                <Dialog
                    ...
                    <div className="w-full h-full px-4 pb-5">
                        {/* {this.state.sequence === 0 ? */}
                        <ForeignShipment data={this.state.single_data} isEdit={true} handleClose={() => this.setState({ approveOpen: false })} submitOpen={() => this.setState({ forwardOpen: true })} storeData={this.handleDataStore} handleReject={() => this.setState({ rejectOpen: true })} />
                        {/* //     :
                ...
                <ConfirmationDialog
                    text="Are you Sure to forward the WDN?"
