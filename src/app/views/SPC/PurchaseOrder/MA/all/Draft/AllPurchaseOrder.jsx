...
export default function AllPurchaseOrder() {
    ...
    const [search, setSearch] = useState(pageData?.filterIndentNo)
    ...
    useEffect(() => {
        if (pageData) {
            ...
            const param = {
                ...
                page: pageData.page,
                // status: 'Draft',
                status: ['Pending Approval', 'SUPERVISOR APPROVED', 'SPC APPROVED', 'CANCELED', 'SUPERVISOR REJECTED', 'REJECTED', 'AMENDED'],
                po_type: pageData.POType ? pageData.POType === 'F' ? ["F", "Foreign"] : ["Local", 'L'] : pageData.POType,
                status: pageData.POStatus,
                ...
                <Grid container spacing={2} className="py-1">
                    ...
                    <Grid item lg={3} md={4} sm={6} xs={12}>
                        <FormControl className="w-full">
                            ...
                            <Autocomplete
                                className="w-full"
                                value={POStatus ? { label: POStatus } : null}
                                options={appConst.po_status}
                                onChange={(event, value) => {
