...
class HospitalUser extends Component {
    ...
    async saveVehicleUser() {
        ...
        if (res1.status == 201) {
            if (formData.type == 'MRO' || formData.type == 'Chief Pharmacist' || formData.type == 'Accounts Clerk Hospital') {
    ...
    async loadData() {
        ...
        if (this.state.user_roles.includes("Sales Hospital Admin")) {
            let filterData=this.state.filterData
            filterData.type=appConst.hospital_user_type_for_saleAdmin.map(a => a.type)
            
            this.setState({ hospital_user_type:appConst.hospital_user_type_for_saleAdmin,filterData })
        }else if(this.state.user_roles.includes("Blood Bank Admin")){
            let filterData=this.state.filterData
            filterData.type=appConst.hospital_user_type_for_BloodBankAdmin.map(a => a.type)
            
            this.setState({ hospital_user_type:appConst.hospital_user_type_for_BloodBankAdmin,filterData })
            
        }else{
            let filterData=this.state.filterData
            filterData.type=appConst.hospital_user_type.map(a => a.type)
            
            this.setState({ hospital_user_type:appConst.hospital_user_type,filterData })
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <Autocomplete
                                            ...
                                            value={this.state.hospital_user_type.find(
                                                (v) => v.type === this.state.formData.type
                                            )}
