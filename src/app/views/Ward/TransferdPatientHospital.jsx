...
class TransferdPatientHospital extends Component {
    constructor(props) {
        super(props)
        this.state = {
            drawerOpen: true,
            ...
            columns: [
                        ...
                        customBodyRender: (value, tableMeta, updateValue) => {
                            return (
                                <Grid className='flex items-center'>
                                    <Tooltip title='Admit'>
                                        <IconButton
                                            disabled={(this.state.data[tableMeta.rowIndex].status == 'Admitted') || (this.state.data[tableMeta.rowIndex].status == 'Discharged')}
                                            onClick={() => {
                                                console.log('clicked')
                                        ...
                                        >
                                            <PersonAddIcon
                                                style={{ color: ((this.state.data[tableMeta.rowIndex].status == 'Admitted') || (this.state.data[tableMeta.rowIndex].status == 'Discharged') || (this.state.data[tableMeta.rowIndex].status == 'Transfer')) ? "#bdbdbd" : "#FFAF38" }}
                                            />
                                        </IconButton>
                                    </Tooltip>
                                    ...
                                    <Tooltip title='Discharge'>
                                        ...
                                        >
                                            <CancelIcon
                                                style={{ color: (this.state.data[tableMeta.rowIndex].status == 'Discharged') ? "#bdbdbd" : "#FF3D57" }}
                ...
                {
                    name: 'Transfered Hospital',
                    label: 'Transfered Hospital',
                    options: {
                        display: true,
                        customBodyRenderLite: (dataIndex) => {
                            return this.state.data[dataIndex].to_hospital?.name
                        }
                    },
                },
                {
    ...
    async searchPatients() {
        ...
        let newParams = {
            ...formData, ...{
                ...
                status: 'Discharged',
                discharge_mode: 'Transfer to other hospital'
            }

        }
        //formData.transfer_from=formData.ward_id;
...
export default withStyles(styleSheet)(TransferdPatientHospital)
