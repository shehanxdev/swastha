...
class VehicleUser extends Component {
    ...
    async saveVehicleUser() {
        ...
        if (res.status == 201) {
            this.setState({ alert: true, message: "Vehicle User Save Successful", severity: 'success' })
            this.loadData();
            this.resetFields();
        } else {
            this.setState({ alert: true, message: "Vehicle User Save Unsuccessful", severity: 'error' })
        }
    }
    ...
    render() {
        return (
            <Fragment>
                <MainContainer>
                    <LoonsCard>
                        <CardTitle title={"Add Vehicle User"} />
