...
class StockAvailabilty extends Component {
    ...
    async loadStockData() {
        ...
        if (res.status) {
            ...
            const totalQuantity = res?.data?.posted?.data?.reduce((total, item) => total + parseInt(item.quantity), 0);
            this.setState({
                ourQty: totalQuantity,  
                loaded: true,
            })
        }
    }
                    ...
                    <table className="w-full" border="1" cellpadding="1" cellspacing="1" style={{borderColor:'white'}} >
                        ...
                        <tbody>
                            ...
                            <tr style={{width:'100%'}}>
                                ...
                                <td style={{textAlign:'center'}}>{this.state.ourQty ? convertTocommaSeparated(Number(this.state.ourQty) || 0, 2) : 0}</td>
