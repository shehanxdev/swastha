...
class RMSDItemWarehouse extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            columns: [
                {
                    name: 'sr_number', // field name in the row object
                    label: 'SR Number', // column title that will be shown in table
                    options: {
                        // filter: true,
                        customBodyRender: (value, tableMeta, updateValue) => {
                            // console.log("tableMeta", tableMeta);
                                let data = this.state.data[tableMeta.rowIndex]?.ItemSnap?.sr_no
                                return <p>{data}</p>
                ...
                },
                {
                    ...
                    options: {
                        ...
                        customBodyRender: (value, tableMeta, updateValue) => {
                            // console.log("tableMeta", tableMeta);
                                let data = this.state.data[tableMeta.rowIndex]?.ItemSnap?.medium_description
                                return <p>{data}</p>
                            ...
                            return (
                                ...
                                <Grid container={2}>
                                    ...
                                    <Tooltip title="View">
                                            <IconButton
                                                onClick={() => {
                                                    window.location.href = `/estimation/rmsd--item-/${id}`
                                                }}>
    ...
    async loadItemWarehouse() {
        ...
        let res = await WarehouseServices.getAllWarehousewithOwner(filterData,owner_id)
        if (res.status == 200) {
    ...
    async loadData() {
     ...
     async loadItemEstimation() {
        ...
        // filterData.item_id = this.props.item_id
        // filterData.all_rmsd_warehouses= true
        console.log('FilterData',this.state.formData)
        let res = await EstimationService.getAllEstimationITEMS(this.state.formData)
        if (res.status == 200) {
    ...
    async loadWarehouses() {
        ...
        if (!selected_warehouse_cache) {
            ...
            let formData = this.state.formData
            formData.owner_id = selected_warehouse_cache.owner_id
        ...
        let params = { owner_id:  this.state.formData.owner_id , load_rmsd:true}
    ...
    componentDidMount() {
        // console.log('ID2',this.props.id)
        // console.log('ID3',this.props.item_id)
                        ...
                        <Grid container spacing={2}>
                            <Grid item lg={12} xs={12} style={{ display: 'flex', justifyContent: 'space-between' }}>
                               {/* {this.state.loaded?<div>
                                <Typography variant="h6" className="font-semibold">Item Name: {this.state.data[0]?.ItemSnap?.medium_description}</Typography> 
                            ...
                            <ValidatorForm
                                className="pt-2"
                                onSubmit={() => this.loadItemEstimation()}
                                onError={() => null}>
                                {/* Main Grid */}
                                    ...
                                    <Grid item="item" xs={12} sm={12} md={12} lg={12}>
                                        ...
                                        disableClearable className="w-full"
                                                    ...
                                                    onChange={(e, value) => {
                                                        if (value != null) {
                                                            let formData = this.state.formData
                                                            formData.warehouse_id = value.warehouse_id
...
export default RMSDItemWarehouse;