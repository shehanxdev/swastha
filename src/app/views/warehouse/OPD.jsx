...
class OPD extends Component {
                                    ...
                                    <Tooltip title="View">
                                        <IconButton
                                            ...
                                            onClick={() => {
                                                ...
                                                let selectOPD = this.state.data[tableMeta.rowIndex]
                                                window.location.href=`/hospital-data-setup/create-new-OPD/${this.state.data[tableMeta.rowIndex].id}` + "?owner_id=" + this.state.owner_id + "&type=" + selectOPD.Department.name + "&name=" + selectOPD.name + "&clinic_no_prefix=" + selectOPD.clinic_no_prefix + "&store_id=" + selectOPD.store_id;
                                                // var createClinicWindow = window.open(`/hospital-data-setup/create-new-OPD/${this.state.data[tableMeta.rowIndex].id}` + "?owner_id=" + this.state.owner_id + "&type=" + selectOPD.Department.name + "&name=" + selectOPD.name + "&clinic_no_prefix=" + selectOPD.clinic_no_prefix + "&store_id=" + selectOPD.store_id, '_blank');
                                            }}
                                            size="small"
                                            aria-label="view"
                                        >
 ...
 handleUpdate = (val) => {
    ...
    async saveOPD() {
        ...
        let res = await PharmacyService.createPharmacy(OPDDTO, this.state.owner_id)
        if (201 == res.status) {
           // this.clearFields()
            this.setState({
                alert: true,
                message: 'OPD Created Successfuly',
                severity: 'success',
            }, () => {
                //this.loadData()
            ...
            this.setState({
                alert: true,
                message: 'OPD creation was Unsuccessful',
                severity: 'error',
            })
        }
    }
        ...
        async updateOPD() {
            ...
            let res = await PharmacyService.updateOPD(this.state.owner_id, id, OPDDTO);
            if (res.status) {
                this.setState({
                    alert: true,
                    message: 'OPD update Successfuly',
                    severity: 'success',
                }, () => {
                    // this.loadData()
                      window.location.reload()
                ...
                this.setState({ alert: true, message: "OPD Update Unsuccessful", severity: 'error' })
            }
        }
        ...
        return (
            <Fragment>
                <MainContainer>
                    ...
                    <LoonsCard>
                        <CardTitle title="Add New OPD" />
                        
                        <div className="w-full">
                        {/* {this.state.loaded( */}
                            <ValidatorForm
                                className="pt-2"
                                onSubmit={() => this.state.buttonName == "Save" ? this.saveOPD() : this.updateOPD()}
                                // onSubmit={() => this.saveOPD()}
                                onError={() => null}
                            >
                        ...
                        :null}
                                    ...
                                    >
                                        ...
                                        disableClearable
                                            ...
                                            //             .OPD_type
                                            // )}
                                            value = {{name:this.state.formData.OPD_type}}
                                            ...
                                            renderInput={(params) => (
                                                <TextValidator
                                                    ...
                                                    variant="outlined"
                                                    size="small"
                                                    // value={this.state.formData.OPD_type}
                                                />
                                    ...
                                    <Grid
                                        ...
                                        xs={12}
                                    >
                                        <SubTitle title="OPD ID" />

                                        <TextValidator
                                            className=" w-full"
                                            placeholder="Ex:OPD000001"
                                            name="store_id"
...
export default withStyles(styleSheet)(OPD)
