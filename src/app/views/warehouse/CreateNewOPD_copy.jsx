...
class CreateNewOPD extends Component {
    constructor(props) {
        super(props)
        this.state = {
            empLoaded: false,
    ...
    async loadData() {
        ...
        let allOPDs = await PharmacyService.getPharmacyById(this.props.match.params.id, owner_id, {
            limit: 20,
            page: 0,
            issuance_type: 'OPD'
        })

        console.log("all OPD: ", allOPDs);

        if (allOPDs.status == 200) {
            console.log(allOPDs)
            this.setState({
                loaded: true,
                empLoaded: true,
                //data: allOPDs.data.view,
                empData: allOPDs.data.view.AssignedEmployees,
                totalPages: allOPDs.data.view.totalPages,
                totalItems: allOPDs.data.view.totalItems,
            })
    ...
    render() {
        ...
        return (
            <Fragment>
                <MainContainer>
                    <LoonsCard>
                        <CardTitle title={this.state.name + "\tOPD"} />
...
export default withStyles(styleSheet)(CreateNewOPD)
