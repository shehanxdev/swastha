...
class AssingDrugStore extends Component {
    constructor(props) {
        super(props)
        this.state = {
            ...
            data: {},
            employeeFilterData: {
                type: ['Drugstore Pharmacist(S)','Blood Bank MLT','Drug Store Keeper','Medical Laboratory Technologist','Radiographer', 'Chief MLT', 'Chief Radiographer', 'Drug Store Officer', 'RMSD MSA']
            },
            filterData: {
                page: 0,
                limit: 20,
            ...
            empType: [
                {
                    type: 'Drug Store Keeper',
                    label: 'Drug Store Pharmacist'
                },
                {
                    type: 'Chief MLT',
                    label: 'Chief MLT'
                ...
                {
                    type: 'Chief Radiographer',
                    label: 'Chief Radiographer'
                },
                { type: 'Medical Laboratory Technologist', label: 'Medical Laboratory Technologist' },
                { type: 'Radiographer', label: 'Radiographer' },
                { type: 'Development Officer', label: 'Development Officer' },
                { type: 'ECG Technician', label: 'ECG Technician' },
                { type: 'EEG Technician', label: 'EEG Technician' },
                { type: 'Public Health Management Assistant', label: 'Public Health Management Assistant' },
                

                {
                    type: 'RMSD MSA',
                    label: 'RMSD Drug Store Pharmacist'
                }

            ],
            empType_for_SalesAdmin: [
                { type: 'Drugstore Pharmacist(S)', label: 'Drugstore Pharmacist(S)' },
                {
                    type: 'Chief MLT',
                    label: 'Chief MLT'
                },
            ...
            empType_for_BloodBankAdmin: [
                { type: 'Blood Bank MLT', label: 'Blood Bank MLT' },
                { type: 'Blood Bank Chief MLT', label: 'Blood Bank Chief MLT' },
                { type: 'Blood Bank Consultant', label: 'Blood Bank Consultant' },
                { type: 'Blood Bank MLT (NOIC)', label: 'Blood Bank MLT (NOIC)' },
    ...
    async componentDidMount() {
        ...
        const user_info = await localStorageService.getItem('userInfo')
        if (user_info?.roles.includes("Sales Hospital Admin")) {
            this.setState({ empType:this.state.empType_for_SalesAdmin })
        }

        if (user_info?.roles.includes("Blood Bank Admin")) {
            this.setState({ empType:this.state.empType_for_BloodBankAdmin })
        }
...
export default withStyles(styleSheet)(AssingDrugStore)
