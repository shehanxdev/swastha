...
class IndividualPurchaseDetails extends Component {
                        ...
                        customBodyRenderLite: (dataIndex) => {
                            if (this.state.data) {
                                return <input
                                    ...
                                    onClick={()=>{this.clickingSupplier(this.state.supplier_data[dataIndex]?.id)}}
                                />
                            } else {
                                return "N/A"
                            }
    ...
    loadData = async () => {
        ...
        if (res.status === 200) {
            this.setState({ data: res.data.view },()=>{
                ...
                this.loadHistory(res?.data?.view?.id)
            });
    ...
    async loadAllSuppliers(value) {
        this.setState({ supplier_loading: false })
    ...
    loadHistory = async (id) => {
        console.log("History Data is :", id)
        this.setState({ history_loading: false })

        let res = await LocalPurchaseServices.getLPRequestApprovals({ lp_request_id: id, 'order[0]': ['sequence', 'ASC'] });
        if (res.status === 200) {
                ...
                <MainContainer>
                    ...
                    <LoonsCard>
                        ...
                        <div style={{ display: "flex", flexDirection: "row" }}>
                            ...
                            {this.state.loading && !this.state.data.status === "REJECTED" &&
                                <div>
                                    <Button
                        ...
                        <ValidatorForm onSubmit={()=>{this.saveQuotation()}}>
                            <Grid container spacing={2}>
                                <Grid
                                    ...
                                    xs={12}
                                >
                                    <SubTitle title="Closing Date & Time" />
