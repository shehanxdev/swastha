...
class ApprovedLPRequest extends Component {
    constructor(props) {
        super(props)
        this.state = {
            activeStep: 1,
                    ...
                    options: {
                        ...
                        customBodyRender: (value, tableMeta, updateValue) => {
                            ...
                            return (
                                <>
                                    {
                                        (this.state.userRoles.includes('Drug Store Keeper') || this.state.userRoles.includes('RDHS') || this.state.userRoles.includes('Medical Laboratory Technologist') || this.state.userRoles.includes('Radiographer') || this.state.userRoles.includes('Devisional Pharmacist') || this.state.userRoles.includes('Accounts Clerk RMSD') || this.state.userRoles.includes('Accounts Clerk Hospital')) ?  
                                            <Grid container>
                                                ...
                                                <Grid item lg={3} md={4} sm={6} xs={12}>
                                                    {
                                                        ...
                                                        <Tooltip title="Create Quotation">
                                                            <IconButton
                                                                ...
                                                                onClick={() => {
                                                                    if (cost < 2e5) {
                                                                        window.location = `/localpurchase/request/${id}?create_quotation=true&is_shopping=true`
                                                                    } else {
                                                                        window.location = `/localpurchase/request/${id}?create_quotation=true&is_normal=true`
                                                                    }
                                                                }}>
                                                ...
                                                <Grid item lg={3} md={4} sm={6} xs={12}>
                                                    <Tooltip title="Add Price">
                                                        <IconButton
                                                            ...
                                                            onClick={() => {
                                                                if (cost < 2e5) {
                                                                    window.location = `/localpurchase/add_prices/${id}?is_shopping=true`
                                                                } else {
                                                                    window.location = `/localpurchase/add_prices/${id}?is_normal=true`
                                                                }
                                                                // window.location = `/localpurchase/add_prices/${id}`
                                                            }}>
    ...
    loadData = async () => {
        ...
        let res = await LocalPurchaseServices.getLPRequest({ ...formData, owner_id: owner_id, status: ['APPROVED'] })
    ...
    async loadSuppliersList() {
        console.log("cheking oricing incom id:", this.state.selected_id)
        let params  = {
            // status : ['Pending', 'Director Approve'],
            'order[0]': ['unit_price', 'ASC'],
            lp_request_id : this.state.selected_id
        }
        ...
        if (res.status === 200){
            ...
            console.log("cheking oricing data: fil", filteredData)
            this.setState({
                supplierDataList:filteredData
            })
        }
    }
    ...
    async grouping() {
        ...
        groupedData.push(
            {
                ...
                status: "Active",
                quantity: Number(this.state.formSingleData?.order_qty),
                standard_cost:  this.state.supplierInfo.unit_price ? this.state.supplierInfo.unit_price : null,
                type: data?.type ? data?.type : null,
                order_date: data?.order_date ? data?.order_date : null,
                order_date_to: data?.order_date_to ? data?.order_date_to : null,
                short_description: data?.ItemSnap?.short_description ? data?.ItemSnap?.short_description : null,
                sr_no: data?.ItemSnap?.sr_no ? data?.ItemSnap?.sr_no : null,
                order_quantity: data?.quantity ? Number(data.quantity) : null,
                pending_quantity: data?.quantity && data?.allocated_quantity ? Number(data?.quantity) - Number(data?.allocated_quantity) : null,
        ...
        formData.category_id = data?.ItemSnap?.Serial?.Group?.category_id ? data?.ItemSnap?.Serial?.Group?.category_id : null
        formData.order_for_year = data?.required_date ? new Date(data?.required_date).getFullYear() : null
        formData.order_date = data?.required_date ? data?.required_date : null
        formData.status = data?.status ? data?.status : null
    ...
    async addNewSchedule(index, scheduleIndex) {
        let formData = this.state.formSingleData
        if (formData.order_items[index].schedule[scheduleIndex]?.schedule_date && formData.order_items[index].schedule[scheduleIndex]?.quantity && formData.order_items[index].schedule[scheduleIndex]?.quantity > 0) {
            ...
            this.setState({
                message: "Please fill out the previous inputs",
                severity: 'warning',
                alert: true
            })
        }
    ...
    async submitData() {
        ...
        for (let i = 0; i < formData.order_items[0].schedule.length; i++) {
            sum += parseInt(formData.order_items[0].schedule[i].quantity, 10);
        }

        let user_id = await localStorageService.getItem('userInfo').id
        let owner_id = await localStorageService.getItem('owner_id')
        let role = await localStorageService.getItem('userInfo').roles[0]

        console.log('cheking single sum', sum)
        console.log('cheking single data', this.state.single_data)
        console.log('cheking single supplier', this.state.supplierInfo)
        formData.order_no = this.state.single_data?.request_id;
        formData.lp_request_id = this.state.single_data?.id
        formData.estimated_value = this.state.supplierInfo?.unit_price ? this.state.supplierInfo?.unit_price * sum : null
        formData.order_date_to = formData.order_items[0].schedule ? formData.order_items[0].schedule[formData.order_items[0].schedule.length - 1].schedule_date : null
        formData.created_by = user_id;
        formData.owner_id = this.state?.single_data?.owner_id;

        // if (role === 'Drug Store Keeper') {
        //     formData.owner_id = owner_id;
        // } else if (role === 'RDHS' || role === 'Accounts Clerk RMSD' || role === 'Devisional Pharmacist') {
        //     let ownerIdList = this.state.data.map((dataset) => dataset?.owner_id)
        //     let uniquOwnerIdList = [...new Set(ownerIdList)]
        //     console.log('gddgdgdfddgdfgdgdgdgdgdgd',uniquOwnerIdList)
        //     formData.owner_id = uniquOwnerIdList[0]
        // }

        formData.no_of_items = this.state.formSingleData.order_items.length;
        formData.order_schedule_data = this.state.single_data
        this.setState({ formData })

        console.log('cheking values', this.state.formSingleData.order_qty, sum)
        if (!isNaN(sum) && Number(this.state.formSingleData.order_qty) === Number(sum)) {
            console.log("submitting data", formData)
            console.log('Value: ', this.state.formSingleData.order_items)
            let res = await PrescriptionService.OrdersCreate(formData)

            if (res.status == 200 || res.status == 201) {
                this.setState({
                    message: "LP Purchase Order was placed Successfully",
                    severity: 'success',
                    alert: true
                },()=>{
                    this.setCompleted()
                ...
                this.setState({
                    message: "LP Purchase Order was Unsuccessful",
                    severity: 'error',
                    alert: true
                })
            }
            ...
            this.setState({
                message: "Summation of the Quantity should match",
                severity: 'error',
                alert: true
            })
        }
    ...
    async setCompleted(){
        ...
        let id = this.state.single_data?.id

        let data = this.state.single_data
        data.status = 'PO CREATED'

        console.log('cheking data', data)
        let res = await LocalPurchaseServices.poApproval(id, data)

        if ( res.status === 200) {
            setTimeout(() => {
                window.location.href = '/localpurchase/order_details'
            }, 1200);
        }
    }

    setRemainingQuantity = (index, scheduleIndex) => {
        const formData = this.state.formSingleData;
        const orderQty = this.state.formSingleData.order_qty;
        const schedules = formData.order_items[index].schedule;
        let assignedQuantity = 0;

        schedules.forEach((schedule) => {
            assignedQuantity += schedule.quantity ? parseInt(schedule.quantity) : 0;
        });

        if (schedules[scheduleIndex].quantity && schedules[scheduleIndex].quantity > 0) {
            assignedQuantity -= schedules[scheduleIndex].quantity
        }

        const remainingQuantity = orderQty - assignedQuantity;
        formData.order_items[index].schedule[scheduleIndex].maxQuantity = remainingQuantity;
    ...
    async getApprovedInfo(){
        let params = {
            lp_request_id : this.state.single_data?.id
        }
        console.log("LP aproved qty: ", this.state.single_data)
        ...
        if (res.status === 200) {
            ...
            console.log("LP aproved res: ", res)
               let msd_director = res.data.view.data.filter((item) => item.approval_user_type === 'MSD Director');

               msd_director.forEach((item) => {
                formData.order_qty = item.approved_qty
                this.setState({msdDirector: item.approved_qty, formData})
               });
        }
    ...
    calculateTotalQty(data) {
        ...
        for (let i = 0; i < data.schedule.length; i++) {
            sum += parseInt(data.schedule[i].quantity, 10);
        }
        if (!isNaN(sum)) {
            return <Typography className=" text-gray font-semibold text-13 mt-2" style={{ lineHeight: '1', }}> Total:{sum} / {this.state.formSingleData.order_qty}</Typography>;
        }
    }
                            ...
                            <Table aria-label="customized table">
                                <TableHead>
                                    <TableRow>
                                        ...
                                        <StyledTableCell align="center">Approved&nbsp;Quantity</StyledTableCell>
                                ...
                                {this.state.single_loading && this.state.single_data ?
                                    <TableBody>
                                        <StyledTableRow key={this.state.single_data?.id}>
                                            ...
                                            <StyledTableCell align="center">{this.state.single_data?.approved_qty ? this.state.single_data?.approved_qty : 'Not Available'}</StyledTableCell>
                                            <StyledTableCell align="center" style={{ display: "flex", justifyContent: "center", marginBottom: "6px" }}>
                                                <ValidatorForm>
                                                    <TextValidator
                                                        ...
                                                        errorMessages={[
                                                            'this field is required', 'Quantity Should Greater-than: 0 '
...
export default withStyles(styleSheet)(ApprovedLPRequest)
