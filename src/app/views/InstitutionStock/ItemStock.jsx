...
class ItemStock extends Component {
                                ...
                                <Grid container={2}>
                                    ...
                                    <Grid item>
                                        <Tooltip title="Stock Movement ">
                                            <IconButton
                                                onClick={() => {
                                                    window.location = `/drugbalancing/checkStock/detailedview/${id}?owner_id=${this.props.ownerID}`
                ...
                },
                {
                    ...
                    options: {
                        ...
                        customBodyRender: (value, tableMeta, updateValue) => {
                            ...
                            // stock qty
                            let item_id = this.state.data[tableMeta.rowIndex]?.id;
    ...
    async getConsumptionDet(id, maindata) {
        ...
        if (res.status) {
            updatedArray = maindata.map((obj1) => {
                const obj2 = res.data.view.find((obj) => obj.item_id === obj1.id);
    ...
    async printData() {
        ...
        delete params.limit
        delete params.page
        let res = await InventoryService.fetchAllItems(
            this.state.formData
        )
        let order_id = 0
        if (res.status) {
    ...
    async loadStockData(itemList) {
        console.log('cheking ownndsklkalskd', this.props.ownerID)
        let params = {
            // warehouse_id: this.props.warehouse_id,
            owner_id: this.props.ownerID,
            items: itemList,
