...
class StockDetailedView extends Component {
    constructor(props) {
        super(props)
        this.state = {
            buttonName: 'Save',
            ...
            columns: [
                ...
                },
                {
                    ...
                    options: {
                        customBodyRender: (value, tableMeta, updateValue) => {
                            ...
                            return this.state.all_bin_stocks[tableMeta.rowIndex]?.ItemSnapBatch?.unit_price;
...

                {
                    ...
                    options: {
                        ...
                        download: false,
                        customBodyRender: (value, tableMeta, updateValue) => {
                            let id = this.state.all_bin_stocks[tableMeta.rowIndex]?.ItemSnapBatch?.item_id
                            let ownerId = this.state.all_bin_stocks[tableMeta.rowIndex]?.owner_id
                            let warehouseId = this.state.all_bin_stocks[tableMeta.rowIndex]?.warehouse_id
                            return (
                                <Grid className="flex items-center">
                                    ...
                                    <Tooltip title="Stock Movement">
                                        <IconButton
                                            onClick={() => {
                                                window.location = `/drugbalancing/checkStock/detailedview/${id}?owner_id=${ownerId}&warehouse_id=${warehouseId}`
            ...
            post_column: [
                ...
                // {
                //     name: 'item_unit_size',
                //     label: 'Item Unit Size',
                //     options: {
                //         customBodyRender: (value, tableMeta, updateValue) => {

                //             return this.state.get_all_item_batches[tableMeta.rowIndex]?.ItemSnap?.item_unit_size
                ...
                //                         onChange={(e) => {
                //                             let formData = this.state.formData
                //                             formData.count_quantity = e.target.value

                //                             this.setState({ edit: true, formData })
                //                         }}
                //                         variant="outlined"
                //                         size="small"
                //                         style={{
    ...
    async getCountQuantityValue() {

        const searchParams = new URLSearchParams(window.location.search);
        const id = searchParams.get('id');

        let params = { verification_item_id: id, search_type: 'VerificationQuantity' }
        let res = await StockVerificationService.getCountQuantity(params)
        console.log('count quantity', res)
        console.log("sum of count quantity", res.data.view[0].sum_of_count_quantity)
        this.setState({
            counted_quantity: res.data.view[0].sum_of_count_quantity
        })
    ...
    async getAllBinStocks() {
        ...
        if (res.status == 200) {
            ...
            this.setState({
                ...
                // console.log('uom', res.data.view.data.map(x => x))
                // this.getUOMFroPrint(res.data.view.data.map(x => x))
                    ...
                    <LoonsCard>
                        ...
                        >
                            ...
                            <Card style={cardStyle}>
                                ...
                                <Grid style={centerGridStyle} container spacing={1} className="flex m-5 " alignItems="center">
                                    <Grid
                                        className=" w-full flex mx-1" item lg={3} md={3} sm={12} xs={12} style={{ border: "1px solid #ADD8E6" }}
                                    >
                                        <Grid style={centerGridStyle} className="pb-1 w-full flex" item lg={12}>
                                            <SubTitle title="Batches Counted Quantity:" />
                                            &nbsp;
                                            <SubTitle title={this.state.counted_quantity} />
                                    ...
                                    >
                                        <Grid style={centerGridStyle} className="pb-1 w-full flex" item lg={12}>
                                            <SubTitle title="All Count Quantity:" />
                                            &nbsp;
                                            <SubTitle title={this.state.item_code?.count_quantity} />
                                    ...
                                    >
                                        <Grid style={centerGridStyle} className="pb-1 w-full flex" item lg={12}>
                                            ...
                                            {this.state.counted_quantity !== this.state.item_code?.count_quantity ? (
                                                <Typography style={{ color: 'red' }}>Count quantites are not same!</Typography>


                                            ) : (
                                                <Typography style={{ color: 'blue' }}>Count quantites are equal!</Typography>
...
export default StockDetailedView