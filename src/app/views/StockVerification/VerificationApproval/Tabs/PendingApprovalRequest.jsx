...
class PendingApprovalRequest extends Component {
    constructor(props) {
        super(props)
        this.state = {
            allWarehouses: [],
                                ...
                                <Grid className="flex items-center">
                                    <Tooltip title="View">
                                        <IconButton
                                            className="px-2"
                                            onClick={() => {
                                                window.location.href = `/stock_Verification/approval_item_list/${this.state.stock_verification_data[tableMeta.rowIndex]?.id}?owner_id=${this.state.stock_verification_data[tableMeta.rowIndex]?.owner_id}&freez_id=${this.state.stock_verification_data[tableMeta.rowIndex]?.id}`
                                            }}
                                            size="small"
                                            aria-label="view"
                                        >
    ...
    async loadData() {
        ...
        if (res.status === 200) {
            ...
            console.log("stock verification approved", res.data.view.data)
    ...
    render() {
        return (
            <Fragment>
                <MainContainer>
                    <LoonsCard>
                        <CardTitle title={"Approved Approval"} />
