...
const data = [
    ...
    {
        label: 'Sinuses of the Scrotum',
        value: 'sinusesOfTheScrotum',
    },
]

class Scrotum extends Component {
    constructor(props) {
        super(props)
        this.state = {
            alert: false,
    ...
    async loadData() {
        ...
        if (200 == res.status) {
            console.log("Examination Data Scrotum", res.data.view.data)
            this.setState({ data: [] })
            let data = [];
            let other_answers = [];
                ...
                <ValidatorForm onSubmit={() => { this.submit() }} className='flex mx-2 mt-4' >
                    <Grid container spacing={1}>  
                        ...
                        <Grid className='' item lg={12} md={12} sm={12} xs={12}>
                            <SubTitle title='Scrotum' />
                            <RadioGroup defaultValue={this.state.formData.examination_data[0].other_answers.scrotum} row>
                                <FormControlLabel
                                    label={"No"}
                                    name="no"
                                    value="no"
                                    onChange={() => {
                                        let formData = this.state.formData;
                                        formData.examination_data[0].other_answers.scrotum = "no"
                                ...
                                <FormControlLabel
                                    ...
                                    onChange={() => {
                                        let formData = this.state.formData;
                                        formData.examination_data[0].other_answers.scrotum = "present"
                                        this.setState({ formData })
                                    }}
                                    control={
                                        <Radio size='small' color="primary" />
                        ...
                        {/* Scrotal Skin */}
                        <Grid className='' item lg={12} md={12} sm={12} xs={12}>
                            <SubTitle title='Scrotal Skin' />
                            <RadioGroup defaultValue={this.state.formData.examination_data[0].other_answers.scrotalSkin} row>
                                { data.map((data) => 
                                    <FormControlLabel
                                    label={data.label}
                                    name={data.value}
                                    ...
                                    onChange={() => {
                                        let formData = this.state.formData;
                                        formData.examination_data[0].other_answers.scrotalSkin = data.value
                                        this.setState({ formData })
                                    }}
                                    control={
                                        <Radio size='small' color="primary" />
...
export default withStyles(styleSheet)(Scrotum)
