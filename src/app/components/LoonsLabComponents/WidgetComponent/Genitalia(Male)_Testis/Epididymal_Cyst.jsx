...
class Epididymal_Cyst extends Component {
    constructor(props) {
        super(props)
        this.state = {
            alert: false,
    ...
    async loadData() {
        ...
        if (200 == res.status) {
            console.log("Examination Data Epididymal_Cyst", res.data.view.data)
            this.setState({ data: [] })
            let data = [];
            let other_answers = [];
                ...
                <ValidatorForm onSubmit={() => { this.submit() }} className='flex mx-2' >
                    <Grid container spacing={1}>
                        ...
                        <Grid className='' item lg={12} md={12} sm={12} xs={12}>
                            <SubTitle title='Epididymal Cyst' />
                            <RadioGroup defaultValue={this.state.formData.examination_data[0].other_answers.epididymal_Cyst} row>
                                <FormControlLabel
                                    label={"No"}
                                    name="no"
                                    value="no"
                                    onChange={() => {
                                        let formData = this.state.formData;
                                        formData.examination_data[0].other_answers.epididymal_Cyst = "no"
                                ...
                                <FormControlLabel
                                    ...
                                    onChange={() => {
                                        let formData = this.state.formData;
                                        formData.examination_data[0].other_answers.epididymal_Cyst = "present"
                                        this.setState({ formData })
                                    }}
                                    control={
                                        <Radio size='small' color="primary" />
                        ...
                        {/* If select Abnormal */}
                        { this.state.formData.examination_data[0].other_answers.epididymal_Cyst === 'present' ?
                            <Grid className='' item lg={12} md={12} sm={12} xs={12}>
                                <FormControlLabel
                                // key={i}
                                label='Right'
...
export default withStyles(styleSheet)(Epididymal_Cyst)
