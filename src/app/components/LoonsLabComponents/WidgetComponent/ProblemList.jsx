...
class ProblemList extends Component {
    constructor(props) {
        ...
        this.state = {
            alert: false,
            message: "Problem is added Successfull",
            severity: 'success',
            data: [],
            ...
            columns: [
                {
                    name: 'problem', // field name in the row object
                    label: 'Problem', // column title that will be shown in table
                    options: {
                        filter: false,
                        display: true
                    }
    ...
    async loadData() {
        ...
        let params = {
            ...
            //widget_input_id: this.props.itemId,
            //'question[0]':'problems',
            question: ['problems', 'diagnosis', 'complications'],
            //limit: 20
        }
        ...
        if (200 == res.status) {
            ...
            res.data.view.data.forEach(element => {
                if (element.question == 'problems') {
                    this.state.data.push(element.other_answers)
                } else if (element.question == "diagnosis") {
                    this.state.data.push({ problem: element.other_answers.diagnosis, duration: "" })
                } else if (element.question == "complications") {
                    this.state.data.push({ problem: element.other_answers.complication, duration: element.other_answers.duration })
                }
    ...
    async submit() {
        ...
        if(!patientSummary.problemList){
            patientSummary.problemList = []
        }
        let res = await ExaminationServices.saveData(formData)
        console.log("Examination Data added", res)
        if (201 == res.status) {
            patientSummary.problemList.push(formData.examination_data[0].other_answers.problem)
                                ...
                                <Grid item lg={12} md={12} sm={12} xs={12} className='hide-on-fullScreen' >
                                    <TextValidator
                                        ...
                                        onChange={(e) => {
                                            ...
                                            let formData = this.state.formData;
                                            formData.examination_data[0].other_answers.problem = e.target.value
                                            this.setState({ formData })
                                        }}
                                        value={this.state.formData.examination_data[0].other_answers.problem}
                            ...
                            <Grid container>
                                ...
                                > */}
                                   ...
                                   <Grid className='mt-3 px-5' container spacing={2} >
                                    <Grid item lg={12} md={12} sm={12} xs={12}>
                                    <Typography variant="h6" style={{ fontSize: 16, color: 'black' }}>Add New Problem</Typography>
...
export default withStyles(styleSheet)(ProblemList)
